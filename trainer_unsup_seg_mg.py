...
class Trainer(vgtk.Trainer):
    ...
    def eval(self):
        ...
        if self.pre_compute_delta == 1:
            ...
            set_dr = []
            set_dt = []
            shape_type_to_n_parts = {"eyeglasses": 3, "oven": 2, "washing_machine": 2, "laptop": 2, "drawer": 4}
            ...
            with torch.no_grad():
                for it, data in enumerate(self.dataset):
                    ...
                    feat_save_fn = os.path.join(self.model.module.log_fn,
                                                f"eval_tr_out_feats_{idxes_str}_rnk_{self.local_rank}.npy")
                    np.save(feat_save_fn, out_feats)
                ...
                for i_p in range(n_s):
                    ...
                    # todo: how to fit t? I'm not that clear...
                ...
                if self.pre_compute_delta >= 1:
                    ...
                    for i_bz in range(bz):
                        ...
                        for it_p in range(matching_idx_gt.size(1)):
                            ...
                            # delta
                            cur_bz_cur_match_pred_rot = torch.matmul(cur_bz_cur_match_pred_R, cur_bz_cur_match_delta_R.contiguous().transpose(0, 1).contiguous())
                            ...
                            cur_bz_cur_match_pred_canon_rot = torch.matmul(cur_bz_cur_match_pred_R, cur_bz_cur_match_canon_pred_R.contiguous().transpose(0, 1).contiguous())
                            cur_bz_cur_match_pred_pred_rot = torch.matmul(cur_bz_cur_match_pred_R, cur_bz_cur_match_pred_R.contiguous().transpose(0, 1).contiguous())
                            ...
                            cur_bz_cur_match_diff_t = torch.norm(cur_bz_cur_match_pred_T - cur_bz_cur_match_delta_T - cur_bz_cur_match_gt_trans, dim=-1).item()
                            cur_bz_cur_match_diff_t_2 = torch.norm(cur_bz_cur_match_real_pred_T - cur_bz_cur_match_gt_trans, dim=-1).item()
                        ...
                        for ip_a in range(matching_idx_gt.size(1) - 1):
                            ...
                            for ip_b in range(ip_a + 1, matching_idx_gt.size(1)):
                                ...
                                rel_rot_R = torch.matmul(pred_R_a, safe_transpose(pred_R_b, 0, 1))
            ...
            if self.local_rank == 0:
                ...
                if self.pre_compute_delta >= 1:
                    ...
                    for i_p in part_idx_to_rot_diff:
                        ...
                        log_str += f"part idx: {i_p}, rot_diff: {cur_part_rot_diff}, canon_rot_diff: {cur_part_canon_rot_diff}, posed_posed_diff: {cur_part_pred_pred_diff}, trans_diff: {cur_part_trans_diff}, trans_diff_2: {cur_part_trans_diff_2}\n"
                    ...
                    for cur_part_pair in part_pair_to_part_rel_rot_diff:
                        ...
                        log_str += f"part pair: {cur_part_pair}, rot diff: {avg_curr_rot_diff}\n"
                    ...
                    for i_s in slot_idx_to_rot_diff:
                        ...
                        log_str += f"slot idx: {i_s}, slot_rot_diff: {cur_slot_rot_diff}, canon_slot_rot_diff: {cur_slot_canon_rot_diff}\n"
                    self.logger.log('Testing', log_str)
