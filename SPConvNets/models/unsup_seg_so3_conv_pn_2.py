...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.log_fn = f"out_feats_weq_wrot_{self.global_rot}_equi_{self.use_equi}_model_{self.model_type}_decoder_{self.decoder_type}_num_iters_{self.num_iters}_npts_{self.npoints}_perpart_npts_{self.part_pred_npoints}_bsz_{self.batch_size}_init_lr_{self.init_lr}.npy"
        ...
        self.slot_attention = SlotAttention(num_slots=params['outblock']['k'], dim=self.encoded_feat_dim, na=self.kanchor, hidden_dim=self.encoded_feat_dim)
    ...
    def forward_one_iter(self, x, pose, rlabel=None): # rotation label
        ...
        # input, x.feats: bz x dim x N x na
        # output, rep_slots: bz x n_s x na x dim; attn_ori: bz x n_s x N x na
        rep_slots, attn_ori = self.slot_attention(x.feats.contiguous().permute(0, 2, 3, 1).contiguous())
        ...
        # hard_one_hot_labels: bz x N x na x n_s
        hard_one_hot_labels = torch.eye(self.num_slots, dtype=torch.float32).cuda()[point_label]
        # hard_one_hot_labels: bz x n_s x N x na
        hard_one_hot_labels = safe_transpose(safe_transpose(hard_one_hot_labels, 2, 3), 1, 2)
        # hard_one_hot_labels_slot: bz x n_s x N x na ---- weights from each slot to each point
        hard_one_hot_labels_slot = hard_one_hot_labels / torch.clamp(hard_one_hot_labels.sum(dim=-2, keepdim=True), min=1e-9)

        # attn: bz x N x n_s x na
        attn = attn_ori.contiguous().transpose(1, 2).contiguous()
        # slot_weights: bz x num_slots x na
        ...
        # transformed_variant_feats: bz x dim x 1 x N x na xxxx bz x 1 x n_s x N x 1
        variant_feats_slot = torch.sum(transformed_variant_feats.unsqueeze(2) * hard_one_hot_labels_slot.unsqueeze(1),
                                       dim=3)
        # bz x 3 x num_slot
        ...
        # recon_slot_points: bz x dim x n_s x na
        ...
        # print(f"recon_slot_points.size: {recon_slot_points.size()}")
        recon_slot_points = recon_slot_points.contiguous().transpose(1, 2).contiguous().transpose(2, 3).contiguous().view(bz, self.num_slots, self.kanchor, self.recon_part_M, -1)
        ...
        # transformed_slot_pts: bz x num_slots x na x M x 3; bz x n_s x na x 3 x 3    xxxx    bz x n_s x na x M x 3; bz x
        ...
        #  bz x n_s x M x N x na; hard_one_hot_labels: bz x n_s x N x na ---> bz x ns x M x N x na
        ...
        # dist_chamfer_recon_ori:  bz x ns x N x na
        ...
        # bz x na
        avg_slots_pts_dist_to_shp = torch.sum(dist_chamfer_recon_slot * slot_weights, dim=-2) # .mean()
        # bz x na
        ...
        sampled_recon_pts = expanded_recon_slot_points.contiguous().view(
            bz * self.kanchor * (self.recon_part_M) * (self.num_slots), -1)[fps_idx, :].contiguous().view(bz * self.kanchor, npoints, -1)
        ...
        # print("minn_orbit_idx:")
        # print(torch.max(minn_orbit_idx), torch.min(minn_orbit_idx))
        # print("jhh")



        selected_point_labels = safe_transpose(point_label, 1, 2)
        # selected_point_labels: bz x N
        selected_point_labels = batched_index_select(values=selected_point_labels, indices=minn_orbit_idx, dim=1)
        ...
        ori_selected_recon_slot_pts = safe_transpose(ori_recon_slot_points, 1, 2)
        ori_selected_recon_slot_pts = batched_index_select(values=ori_selected_recon_slot_pts, indices=minn_orbit_idx, dim=1)
        ...
        expanded_attn = safe_transpose(safe_transpose(attn, 2, 3), 1, 2)
        # selected_attn: bz x N x n_s
        selected_attn = batched_index_select(expanded_attn, minn_orbit_idx, dim=1).squeeze(1)
