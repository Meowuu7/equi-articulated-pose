...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.whole_shp_recon_net = nn.Sequential(
            nn.Conv2d(in_channels=self.encoded_feat_dim, out_channels=1024, kernel_size=(1, 1),
                      stride=(1, 1), bias=True),
            nn.BatchNorm2d(num_features=1024),
            nn.LeakyReLU(inplace=True),
            ...
            nn.BatchNorm2d(num_features=1024),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(in_channels=1024, out_channels=self.npoints * 3, # reconstruct the whole shape
                      kernel_size=(1, 1), stride=(1, 1), bias=True),
            nn.Sigmoid()
        )
        ...
        self.axis_prior_slot_pairs = torch.zeros((3, 3), dtype=torch.float32, requires_grad=False).cuda()
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        # input_x = x # preprocess input and equivariant # equivariant
        ...
        # From x to global feature at each orbit, and variant features
        # x_out: bz x dim;
        # var_out_feats: bz x dim x na
        # orbit_attn_weights: bz x na
        x_out, orbit_attn_weights, var_out_feats = self.glb_outblock(x)
        # global rotation attention weights
        # orbit_attn_weights: bz x na
        if len(orbit_attn_weights.size()) == 1:
            orbit_attn_weights = orbit_attn_weights.unsqueeze(0)

        # psoe
        # rots = gt_pose_segs[:, 0, :3, :3]

        # rots = gt_pose_segs[:, 1, :3, :3]

        # self.anchors: na x 3 x 3
        # rots: bz x N x 3 x 3
        # dot_rots_anchors: bz x na x 3 x 3
        #

        rots = torch.eye(3, dtype=torch.float).unsqueeze(0).repeat(bz, 1, 1).cuda()
        dot_rots_anchors = torch.matmul(rots.unsqueeze(1),
                                        safe_transpose(self.anchors, -1, -2).unsqueeze(0))
        dot_traces = dot_rots_anchors[..., 0, 0] + dot_rots_anchors[..., 1, 1] + dot_rots_anchors[..., 2, 2]
        # gt_orbit: bz  # we can use
        gt_orbit = torch.argmax(dot_traces, dim=-1)
        # gt_orbit: bz
        # gt_orbit = gt_orbit[:, 0]

        oribt_selection_loss = nn.functional.nll_loss(nn.functional.log_softmax(orbit_attn_weights, dim=1), gt_orbit)
        oribt_selection_loss = oribt_selection_loss.mean()


        # x_out: bz x dim
        recon_whl_shape = self.whole_shp_recon_net(x_out.unsqueeze(-1).unsqueeze(-1))
        # recon_whl_shape: bz x (npoints * 3) x 1 x 1
        recon_whl_shape = recon_whl_shape.squeeze(-1).squeeze(-1)
        # recon_whl_shape: bz x npoints x 3
        recon_whl_shape = recon_whl_shape.contiguous().view(bz, self.npoints, 3).contiguous()

        # recon_whl_shape: bz x npoints x 3
        # todo: add orbit attn weights
        # glb_selected_orbit: bz
        glb_selected_orbit = torch.argmax(orbit_attn_weights, dim=-1)
        # glb_selected_var_feats: bz x 1 x dim
        glb_selected_var_feats = batched_index_select(values=safe_transpose(var_out_feats, 1, 2), indices=glb_selected_orbit.unsqueeze(-1), dim=1)
        # glb_selected_anchor: bz x 1 x 3 x 3 --> bz x 3 x 3
        glb_selected_anchor = batched_index_select(values=self.anchors, indices=glb_selected_orbit.unsqueeze(-1), dim=0).squeeze(1)
        # glb_selected_var_feats: bz x 1 x dim --> bz x dim
        glb_selected_var_feats = glb_selected_var_feats.squeeze(1)
        # glb_transformation: bz x 7 x 1 x 1
        glb_transformation = self.glb_transformation_prediction(glb_selected_var_feats.unsqueeze(-1).unsqueeze(-1))
        # glb_transsformation: bz x 7
        glb_transformation = glb_transformation.squeeze(-1).squeeze(-1)
        glb_res_R, glb_res_T = glb_transformation[:, :4], glb_transformation[:, 4:]
        glb_pred_qw, glb_pred_qxyz = torch.split(glb_res_R, [1, 3], dim=-1)
        ...
        glb_res_R = compute_rotation_matrix_from_quaternion(glb_res_R).contiguous().view(bz, 3, 3)
        glb_R = torch.matmul(glb_selected_anchor, glb_res_R)

        glb_R = rots
        # print(rots[0, :3])

        # ''' From globla rotation for a '''
        # bz x 1 x 3 x 3 xxxx 1 x na x 3 x 3 ---> bz x na x 3 x 3
        glb_R_dot_anchors = torch.matmul(safe_transpose(glb_R, -1, -2).unsqueeze(1), self.anchors.unsqueeze(0))

        ''' Stra #1 '''
        # ax_x, ax_y, ax_z = glb_R_dot_anchors[..., 2, 1] - glb_R_dot_anchors[..., 1, 2], glb_R_dot_anchors[..., 0, 2] - glb_R_dot_anchors[..., 2, 0], glb_R_dot_anchors[..., 1, 0] - glb_R_dot_anchors[..., 0, 1]
        # # glb_R_dot_anchors_axises: bz x na x 3
        # glb_R_dot_anchors_axises = torch.cat([ax_x.unsqueeze(-1), ax_y.unsqueeze(-1), ax_z.unsqueeze(-1)], dim=-1)
        # glb_R_dot_anchors_axises = glb_R_dot_anchors_axises / torch.clamp(torch.norm(glb_R_dot_anchors_axises, dim=-1, keepdim=True, p=2), min=1e-8)
        ''' Stra #1 '''

        ''' Stra #2 '''
        # bz x na x 3 x 3
        glb_R_dot_anchors_axises = self.from_rotation_mtx_to_axis(glb_R_dot_anchors)
        ''' Stra #2 '''

        y_axis = torch.tensor([0.0, 1.0, 0.0], dtype=torch.float32).cuda()
        # dot_anchors_axises_y_axis: bz x na
        dot_anchors_axises_y_axis = torch.sum(glb_R_dot_anchors_axises * y_axis.unsqueeze(0).unsqueeze(0), dim=-1)

        # dot_anchors_axises_y_axis = torch.abs(dot_anchors_axises_y_axis)
        # selected_anchors: bz x 5 ---> anchor index
        selected_anchor_dots, selected_anchors = torch.topk(dot_anchors_axises_y_axis, dim=-1, k=5, largest=True)

        # bz x 5 x 3 x 3
        selected_glb_anchors_axises = batched_index_select(values=glb_R_dot_anchors_axises, indices=selected_anchors.long(), dim=1)

        # print(selected_anchor_dots)
        # print(selected_anchors)
        # print(selected_glb_anchors_axises)
        torch.cuda.empty_cache()

        # x.feats: bz x dim x N x na
        # from points' features to global features at different orbits
        ...
        ''' Add entropy loss? --- not a valid loss actually... '''
        ''' Add max percent related loss '''
        ...
        ''' Add the number of activated slots related loss '''
        ...
        # selected_anchor_idxes: bz x 5
        slot_expanded_anchor_idxes = selected_anchors.unsqueeze(1).repeat(1, self.num_slots, 1).contiguous()
        # selected_chamfer_recon_slot_ori: bz x n_s x 5
        selected_chamfer_recon_slot_ori = batched_index_select(values=dist_chamfer_recon_slot_ori, indices=slot_expanded_anchor_idxes, dim=2)
        # dist_chamfer_recon_slot_ori: bz x ns; selected_slot_oribt: bz x ns
        dist_chamfer_recon_slot_ori, selected_slot_oribt = torch.min(selected_chamfer_recon_slot_ori, dim=2)
        # selected_slot_oribt: bz x ns
        selected_slot_oribt = batched_index_select(slot_expanded_anchor_idxes, indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(-1)
        ...
        # dist_chamfer_recon_slot_ori, selected_slot_oribt = self.select_slot_orbits_bak(dist_chamfer_recon_slot_ori, pred_R)
        ...
        ''' Sampled-Ori Chamfer distance hand-crafted calculation version '''
        # bz x npoints x npoints
        glb_dist_recon_ww_ori_ori = torch.sum((sampled_ori_recon_pts.unsqueeze(2) - recon_whl_shape.unsqueeze(1)) ** 2, dim=-1)
        # glb_dist_recon_ww_ori = torch.sqrt(glb_dist_recon_ww_ori)
        glb_dist_recon_to_ori_ori, _ = torch.min(glb_dist_recon_ww_ori_ori, dim=-1)
        glb_dist_ori_to_recon_ori, _ = torch.min(glb_dist_recon_ww_ori_ori, dim=-2)
        ...
        cd_l1_glb_dist_recon_to_ori = torch.sqrt(torch.clamp(glb_dist_recon_to_ori, min=0.0)).mean(dim=-1)
        cd_l1_glb_dist_ori_to_recon = torch.sqrt(torch.clamp(glb_dist_ori_to_recon, min=0.0)).mean(dim=-1)
        # bz
        ...
        self.cd_l1_ori_recon = (cd_l1_glb_dist_ori_to_recon + cd_l1_glb_dist_recon_to_ori) * 0.5
        self.cd_l1.append(self.cd_l1_ori_recon.mean().item())
        print(f"avg_cd_l1: {sum(self.cd_l1) / len(self.cd_l1)}")


        # bz
        # glb_dist_recon_to_ori = glb_dist_recon_to_ori.mean(dim=-1) # .mean()
        glb_dist_recon_to_ori_ori = glb_dist_recon_to_ori_ori.sum(dim=-1)  # .mean()
        # bz
        # glb_dist_ori_to_recon = glb_dist_ori_to_recon.mean(dim=-1) # .mean()
        glb_dist_ori_to_recon_ori = glb_dist_ori_to_recon_ori.sum(dim=-1)  # .mean()
        ...
        print(f"contrained_quat_loss: {regu_quat_loss.item()}, glb_regu_quat_loss: {glb_regu_quat_loss.item()}, oribt_selection_loss: {oribt_selection_loss.item()}")
        ...
        # print(f"axis prediction loss: {axis_pred_axis_loss.item()}")
        # tot_loss = tot_loss + axis_pred_axis_loss
