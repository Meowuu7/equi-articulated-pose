    ...
    def __init__(self, params):
        ...
        for i_s in range(self.num_slots):
            self.pair_slot_outblock.append(
                Mso3.InvOutBlockOursWithMask(params['outblock'], norm=1, pooling_method='attention', use_pointnet=True,
                                             use_abs_pos=use_abs_pos_pair_out, return_point_pooling_feature=return_point_pooling_feature_pair_out)
            )
        ...
        for i_s in range(self.num_slots):
            if self.recon_prior == 4:
                ...
                self.slot_shp_recon_net.append(
                    DecoderFCWithCuboic([256, 256], params['outblock']['mlp'][-1], self.recon_part_M, None, pred_rot=False)
                )
                ...
                self.slot_shp_recon_net.append(
                    DecoderFCWithPVPAtlas([256, 256], params['outblock']['mlp'][-1], self.recon_part_M, None, prior_dim=3)
            ...
            if not self.pred_t:
                ...
                self.slot_trans_outblk_RT.append(
                    cur_slot_trans_outblk(params['outblock'], norm=1, pooling_method='max',
                                          ...
                                          c_in_rot=c_in_rot, c_in_trans=c_in_trans, pred_axis=self.pred_axis, # whehter to predict axis
                                          pred_pv_points=pred_pv_points, pv_points_in_dim=pred_pv_points_in_dim, # whether to predict pv point --> only useful when we use equiv feature for pivot point prediction
                                          pred_central_points=pred_central_points, # whether to predict central points --> only useful when we use equiv feature for central point prediction
                                          central_points_in_dim=pred_central_points_in_dim, #
                                          mtx_based_axis_regression=self.mtx_based_axis_regression) # how to predict axis...
                )
                self.pair_slot_trans_outblk_RT.append(
                    cur_slot_trans_outblk(params['outblock'], norm=1, pooling_method='max',
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if self.stage == 0:
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    if self.zz_pred_pv_equiv:
                        # pair slot outblock...
                        pair_cur_bz_cur_slot_equiv_feats, pair_cur_bz_cur_slot_inv_feats, pair_cur_bz_cur_slot_orbit_confidence = self.pair_slot_outblock[i_s](pair_cur_bz_cur_slot_x, mask=None)
                    else:
                ...
                cur_slot_abs_inv_feats = torch.cat(cur_slot_abs_inv_feats, dim=0) # invariant features encoded using absolute coordinates
                ...
                if self.recon_prior == 5:
                    ...
                    if self.recon_prior in [6, 8]: # reconstruction prior in 6 or 8...
                        cur_slot_canon_pts, cur_slot_pivot_points, cur_slot_central_points = \
                            self.slot_shp_recon_net[i_s](cur_slot_inv_feats, pv_point_inv_feat=cur_slot_abs_inv_feats, central_point_inv_feat=cur_slot_abs_inv_feats)
                    else:
                        cur_slot_canon_pts, cur_slot_pivot_points, cur_slot_central_points = \
                            self.slot_shp_recon_net[i_s](cur_slot_inv_feats)
                    ...
                    # x.xyz: bz x 3 x N
                    pair_cur_slot_canon_pts, cur_slot_pivot_points, _ = self.pair_slot_shp_recon_net[i_s](cur_pair_slot_inv_feats)
            ...
            for i_s in range(len(slot_R)): # get slots' rotations
                ...
                for i_bz in range(bz): # seg label for the base part?
                    ...
                    if i_s == cur_bz_cur_minn_seg_label:
                        ...
                        cur_slot_R_mtx = torch.sigmoid(slot_R[i_s][i_bz].unsqueeze(0)) * math.pi * self.rot_angle_factor # get rotation matrix for the slot
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    if i_s == cur_bz_cur_minn_seg_label: #
                        ...
                        cur_slot_trans = 1.0 * torch.matmul(cur_bz_cur_slot_R, (cur_slot_central_point - cur_slot_pv_point).unsqueeze(-1)) + (cur_slot_pv_point).unsqueeze(-1) #
            ...
            if self.recon_prior == 6 or self.recon_prior == 7 or self.recon_prior == 8 or self.recon_prior == 9:
                ...
                defined_axises = batched_index_select(values=defined_axises, indices=slot_orbits[:, 0].unsqueeze(-1), dim=1).squeeze(1)
                ...
                if self.recon_prior == 7:
                    # dot_normal_axis:
                    dot_normal_axis = get_cuboic_constraint_loss_with_axis_cuboid(slot_pivot_points, slot_central_points, torch.cat(slot_cuboic_recon_pts, dim=1), torch.cat(slot_cuboic_R, dim=1), avg_slot_axis)
                ...
                minn_dist_pv_points_central_transformed_canon_pts, _ = torch.min(dist_pv_points_central_transformed_canon_pts, dim=-1)
                ...
                if self.axis_reg_stra == 1:
                    if self.pv_in_count_nn > 0:
                        ...
                        self.avg_pv_point_distance = (self.avg_pv_point_distance * self.pv_in_count_nn + torch.sum(
                            minn_dist_pv_points_central_transformed_canon_pts)) / (self.pv_in_count_nn + cur_valid_pv_nn)
                ...
                if self.axis_reg_stra == 1:
                    if self.pv_in_count_nn > 0:
                        # valid_pv_points
                        cur_in_count_pv_point_indicators = (minn_dist_pv_points_canon_transformed_canon_pts <= self.avg_pv_point_distance.unsqueeze(0)).float()
                        cur_valid_pv_nn = torch.sum(cur_in_count_pv_point_indicators).item()
                        minn_dist_pv_points_canon_transformed_canon_pts[cur_in_count_pv_point_indicators < 0.5] = 0.0
                        self.avg_pv_point_distance = (self.avg_pv_point_distance * self.pv_in_count_nn + torch.sum(minn_dist_pv_points_canon_transformed_canon_pts)) / (self.pv_in_count_nn + cur_valid_pv_nn)
                        self.pv_in_count_nn = self.pv_in_count_nn + cur_valid_pv_nn
                    else:
                        cur_valid_pv_nn = minn_dist_pv_points_canon_transformed_canon_pts.size(0) * minn_dist_pv_points_canon_transformed_canon_pts.size(1)
                        self.avg_pv_point_distance = torch.sum(minn_dist_pv_points_canon_transformed_canon_pts) / cur_valid_pv_nn
                        self.pv_in_count_nn = self.pv_in_count_nn + cur_valid_pv_nn
                ''' Get and update valid distance... '''
