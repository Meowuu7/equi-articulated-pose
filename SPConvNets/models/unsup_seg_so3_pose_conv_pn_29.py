...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.r_representation = params['general']['r_representation']
        ...
        if self.gt_oracle_seg == 0: #
            ...
            # self.sel_mode = 29 # None # then only the 29-th mode is used for decoding
            # self.sel_mode = None
        ...
        for i_s in range(self.num_slots):
            ...
            #     Mso3.InvOutBlockOursWithMask(params['outblock'], norm=1, pooling_method='attention', use_pointnet=True, sel_mode=29)
            # )
            ###### It is a good outblock and should not be affected by `sel_mode` -> just using
            self.slot_outblock.append(
        ...
        print(f"Using r_representation: {self.r_representation}")
        for i_s in range(self.num_slots):
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if cur_iter > 0:
            ...
            pred_glb_pose = self.pred_glb_pose

            pred_glb_R = pred_glb_pose[:, :3, :3] # use the predicted global rotation as the rotation for the first part... or the largest part? ---> then we need to restrict the rotation range of other parts to along z axis?
            pred_glb_T = pred_glb_pose[:, :3, 3] #
            # transform x via global pose
            x = torch.matmul(safe_transpose(pred_glb_R, -1, -2), (x - pred_glb_T.unsqueeze(-1)))

            if normals is not None:
                # pred_glb_R: bz x 3 x 3 @ normals: bz x 3 x N
                normals = torch.matmul(safe_transpose(pred_glb_R, -1, -2), normals)

            # tmp_R = torch.matmul(safe_transpose(pred_glb_R, -1, -2).unsqueeze(1), pose[:, :, :3, :3])
            # pose[:, :, :3, 3] = torch.matmul(safe_transpose(pred_glb_R, -1, -2).unsqueeze(1), (pose[:, :, :3, 3] - pred_glb_T.unsqueeze(1)).unsqueeze(-1)).squeeze(-1)
            #
            # pose = torch.cat([])
            ############################################# Centralize
            # offset = torch.mean(x, dim=-1, keepdim=True).squeeze(-1).unsqueeze(0).unsqueeze(0)
            # x = x - torch.mean(x, dim=-1, keepdim=True)
            # # print("pose,", pose.size(), offset.size())
            # # print(pose[..., :3, 3].size(), offset.size())
            # pose[..., :3, 3] = pose[..., :3, 3] - offset
            # if gt_pose is not None:
            #     # print("gt_pose,", gt_pose.size(), )
            #     #### Use offset to change gt_pose ####
            #     gt_pose[..., :3, 3] = gt_pose[..., :3, 3] - offset
            # if gt_pose_segs is not None:
            #     # print("gt_pose_segs,", gt_pose_segs.size(), )
            #     #### Use offset to change gt_pose_segs ####
            #     gt_pose_segs[..., :3, 3] = gt_pose_segs[..., :3, 3] - offset
            ############################################# Centralize
        ...
        # cur_kanchor = 60
        cur_kanchor = self.kanchor if cur_iter == 0 else self.kpconv_kanchor
        ...
        if cur_iter == 0:
            ...
            if cur_iter == 0:
                self.glb_canon_pred = {}
                # self.glb_canon_pred[0] = safe_transpose(glb_recon_canon_pts, -1, -2).detach().cpu().numpy()
                self.glb_transformed_pred = {}
                self.glb_ori_x = {}
                self.glb_pred_axis = {}
                # self.glb_transformed_pred[0]
            self.glb_canon_pred[cur_iter] = safe_transpose(glb_recon_canon_pts, -1, -2).detach().cpu().numpy()
            self.glb_transformed_pred[cur_iter] = selected_transformed_glb_recon_pts.detach().cpu().numpy()
            self.glb_ori_x[cur_iter] = safe_transpose(ori_pts, -1, -2).detach().cpu().numpy()
            self.glb_pred_axis[cur_iter] = glb_pred_axis.detach().cpu().numpy()
            self.glb_pred_axis = glb_pred_axis
        else:
            selected_glb_R = torch.eye(3, dtype=torch.float32).cuda().unsqueeze(0).repeat(bz, 1, 1).contiguous()
            selected_glb_T = torch.zeros((bz, 3), dtype=torch.float32).cuda()
            minn_glb_chamfer = torch.zeros((bz,), dtype=torch.float32).cuda()

        # register global pose
        ...
        if self.gt_oracle_seg == 1:
            ...
            if self.sel_mode is not None:
                sel_mode_new = None if cur_iter <= 1 else self.sel_mode_new
            else:
                sel_mode_new = None
            if self.inv_pooling_method == 'attention' and self.sel_mode is None:
        ...
        # from hard labels to hard-one-hot-labels
        ...
        for i_s in range(self.num_slots):
            ...
            #                                        safe_transpose(cur_anchors, -1, -2).unsqueeze(0))
            ...
            for i_bz in range(bz):
                ...
                cur_bz_slot_output_RT = self.slot_trans_outblk_RT[i_s](cur_bz_cur_slot_x, mask=None,
                                                                       anchors=self.anchors.unsqueeze(0).repeat(bz, 1,
                                                                                                                1,
                                                                                                                1).contiguous(), )
        ...
        # just decode a theta and then
        # slot_R: bz x n_s x 4 x na
        if self.r_representation == 'quat':
            slot_R = compute_rotation_matrix_from_quaternion(
                safe_transpose(slot_R, -1, -2).view(bz * cur_kanchor * self.num_slots, -1)).contiguous().view(bz,
                                                                                                              self.num_slots,
                                                                                                              cur_kanchor,
                                                                                                              3, 3)
        else:
            # defined_axis = torch.tensor([0.30353126, 0.9341723, -0.18759232], dtype=torch.float32).cuda().unsqueeze(0)
            ...
            slot_R = torch.sigmoid(slot_R) * math.pi # * (36.0 / 180.0)
        ...
        # Try to select only a certain number of rotations from the predicted rotations #
        # slot_R: bz x n_s x na x 3 x 3
        # slot_R_axises: bz x n_s x na x 3
        ##### Use the obtained rotation matrix to select anchors directly ####
        # slot_R_axises = self.from_rotation_mtx_to_axis(slot_R)
        ##### Use anchors to select orbits ####
        # slot_R_axises = self.from_rotation_mtx_to_axis(cur_anchors.unsqueeze(0).unsqueeze(0).contiguous().repeat(bz, self.num_slots, 1, 1, 1))

        # y_axis = torch.tensor([0.0, 1.0, 0.0], dtype=torch.float32).cuda()
        # if cur_iter == 0:
        #     y_axis = torch.tensor([0.0, 1.0, 0.0], dtype=torch.float32).cuda()
        # else:
        #     y_axis = self.glb_pred_axis # 3
        # dot_product_slot_R_y_axis: bz x n_s x na
        # dot_product_slot_R_y_axis = torch.sum(slot_R_axises * y_axis.unsqueeze(0).unsqueeze(0).unsqueeze(0), dim=-1)
        # norm_slot_R_axises = torch.norm(slot_R_axises, dim=-1, keepdim=False, p=2)
        # dot_product_slot_R_y_axis = dot_product_slot_R_y_axis / torch.clamp(norm_slot_R_axises, min=1e-8)
        # k = 5
        # # k = 3
        # topk_dot_product_slot_R, topk_anchor_idxes = torch.topk(dot_product_slot_R_y_axis, dim=-1, k=k)
        #
        # # k = 6
        # # topk_anchor_idxes = torch.tensor([15, 16, 17, 33, 34, 35], dtype=torch.long).cuda().unsqueeze(0).unsqueeze(0).repeat(bz, self.num_slots, 1).contiguous()
        # #
        # k = 1 # 15, 29 # only use the identity matrix mode #
        # topk_anchor_idxes = torch.tensor([29], dtype=torch.long).cuda().unsqueeze(0).unsqueeze(
        #     0).repeat(bz, self.num_slots, 1).contiguous()
        #

        topk_anchor_idxes = None # no
        k = cur_kanchor

        k = cur_kanchor if self.sel_mode_trans is None else 1
        ...
        # print(f"slot_R: {slot_R.size()}, slot_T: {slot_T.size()}")
        if isinstance(self.slot_trans_outblk_RT[0], SO3OutBlockRTWithMask):
            ...
            # slot_T = avg_offset.unsqueeze(-2).contiguous().repeat(1, 1, cur_kanchor, 1)
        ...
        expanded_hard_one_hot_labels = safe_transpose(hard_one_hot_labels, -1, -2).unsqueeze(2).unsqueeze(2).repeat(1,
                                                                                                                    1,
                                                                                                                    k, # cur_kanchor,
                                                                                                                    self.recon_part_M,
                                                                                                                    1)
        ...
        if self.slot_single_mode == 1:
            ...
            if cur_iter >= 1:
                # self.sel_mode_new = slot_orbits.long()
                if k < cur_kanchor:
                    real_slot_orbits = batched_index_select(topk_anchor_idxes, indices=slot_orbits.unsqueeze(-1), dim=0).squeeze(-1)
                else:
                    real_slot_orbits = slot_orbits.clone()
                self.sel_mode_new = real_slot_orbits.long()
        ...
        if k < cur_kanchor:
            ...
            slot_orbits = batched_index_select(values=topk_anchor_idxes, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(
                -1)
        ...
        if cur_iter > 0:
            ...
            tot_recon_loss = minn_glb_chamfer * 2.0
        # tot_recon_loss = (recon_to_ori_dist + ori_to_recon_dist) * self.glb_recon_factor + (
        # slot_dist_ori_recon) * self.slot_recon_factor + minn_glb_chamfer * 2.0 # add the global reconstruction loss...
        ...
        if cur_iter == 0:
            ...
            self.attn_iter_2 = safe_transpose(hard_one_hot_labels, -1,
                                              -2)  # .contiguous().transpose(1, 2).contiguous().detach()  # .cpu().numpy()
            # self.attn_saved_1 = safe_transpose(hard_one_hot_labels, -1,
            #                                    -2)  # .contiguous().transpose(1, 2).contiguous().detach()
            # self.attn_saved_1 = attn_ori
            self.attn_saved_2 = attn_ori
        ...
        transformed_ori_pts = torch.matmul(selected_inv_pred_R,
                                           (safe_transpose(ori_pts, -1, -2) - selected_pred_T).unsqueeze(-1)).squeeze(
            -1)
        ...
        return tot_loss, selected_attn, pred_pose, out_feats, selected_pts_orbit, gt_pose
    ...
    def forward(self, x, pose, ori_pc=None, rlabel=None, nn_inter=2, pose_segs=None, canon_pc=None, normals=None, canon_normals=None):
        ...
        for i in range(self.num_iters):
            cur_reconstructed_loss_orbit, attn, cur_estimated_pose, cur_out_feats, cur_selected_pts_orbit, cur_gt_pose = self.forward_one_iter(
                cur_transformed_points, cur_estimated_pose, ori_pc=ori_pc, rlabel=rlabel, cur_iter=i,
                gt_pose=cur_gt_pose,
                gt_pose_segs=pose_segs, canon_pc=canon_pc, selected_pts_orbit=cur_selected_pts_orbit, normals=normals, canon_normals=canon_normals)
            torch.cuda.empty_cache()
...
def build_model(opt,
    ...
    params['general'] = {
        ...
        'kpconv_kanchor': opt.equi_settings.kpconv_kanchor,
        'r_representation': opt.equi_settings.r_representation,
