...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.ppint_outblk = Mso3.InvPPOutBlockOurs(params['outblock'], norm=1, pooling_method='max')
        ...
        self.slot_attention = SlotAttention(num_slots=params['outblock']['k'], dim=(self.inv_out_dim + self.kanchor) if self.orbit_attn == 1 else (self.kanchor) if self.orbit_attn == 2 else (self.inv_out_dim + 3) if self.orbit_attn == 3 else (self.inv_out_dim), hidden_dim=self.inv_out_dim, iters=self.slot_iters)
        ...
        for i_s in range(self.num_slots):
            ...
            self.slot_outblock.append(
                Mso3.InvOutBlockOursWithMask(params['outblock'], norm=1, pooling_method='attention')
            )
        ...
        for i_s in range(self.num_slots):
            self.slot_trans_outblk_RT.append(
                SO3OutBlockRTWithMask(params['outblock'], norm=1, pooling_method='max', global_scalar=False, # global scalar?
                              use_anchors=True, feat_mode_num=self.kanchor, num_heads=1)
            )
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        # print(f"after invariant feature processing block: {ppinv_out.size()}")
        ...
        for i_s in range(self.num_slots):
            ...
            if i_s < gt_pose_segs.size(gt_pose_segs.size(1)):
                ...
                dots_gt_rot_anchors = torch.matmul(cur_gt_rot.unsqueeze(1), safe_transpose(self.anchors, -1, -2).unsqueeze(0))
                rel_gt_rot_anchors = dots_gt_rot_anchors[..., 0, 0] + dots_gt_rot_anchors[..., 1, 1] + dots_gt_rot_anchors[..., 2, 2]
                # sel_gt_anchor: bz x
            ...
            if sel_gt_anchor is not None:
                nll_oribt_selection_loss = torch.nn.functional.nll_loss(torch.nn.functional.log_softmax(cur_slot_orbit_confidence, dim=-1), target=sel_gt_anchor)
            ...
            cur_slot_output_RT = self.slot_trans_outblk_RT[i_s](x, mask=cur_hard_one_hot_labels, anchors=self.anchors.unsqueeze(0).repeat(bz, 1, 1, 1).contiguous())
        ...
        slot_R = compute_rotation_matrix_from_quaternion(
            safe_transpose(slot_R, -1, -2).view(bz * self.kanchor * self.num_slots, -1)).contiguous().view(bz,
                                                                                                           self.num_slots,
                                                                                                           self.kanchor,
                                                                                                           3, 3)
        ...
        ### whether to mult anchors with translations? ###
        ### if using RT prediction block, then we should not mult them by anchors again ##
        # slot_T = torch.matmul(self.anchors.unsqueeze(0).unsqueeze(0), safe_transpose(slot_T, -1, -2).unsqueeze(-1)).squeeze(-1)
        ...
        # print(f"After transformed: {transformed_pts.size()}, slot_oribts: {slot_oribts.size()}")
        # transformed_pts: bz x n_s x M x 3
        transformed_pts = batched_index_select(transformed_pts, indices=slot_oribts.unsqueeze(-1), dim=2).squeeze(2)
        # print(f"After orbit selection: {transformed_pts.size()}")
        # slot_canon_pts = batched_index_select(safe_transpose(slot_canon_pts, -1, -2), indices=slot_oribts.unsqueeze(-1), dim=2).squeeze(2)
        # slot_canon_pts: bz x n_s x 3 x M --> bz x n_s x M x 3
        slot_canon_pts = safe_transpose(slot_canon_pts, -1, -2)

        slot_R = batched_index_select(values=slot_R, indices=slot_oribts.unsqueeze(-1), dim=2).squeeze(2)
        slot_T = batched_index_select(values=slot_T, indices=slot_oribts.unsqueeze(-1), dim=2).squeeze(2)

        ### assign ori pts to slot labels ###
        # dist_slot_ori: bz x n_s x M x N
        dist_slot_ori = torch.sum((transformed_pts.unsqueeze(-2) - safe_transpose(ori_pts, -1, -2).unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
        ...
        expanded_hard_slot_indicators = hard_slot_indicators.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, self.recon_part_M, npoints)
        ...
        expanded_ori_pts_slot_one_hot_labels = safe_transpose(ori_pts_slot_one_hot_labels, -1, -2).unsqueeze(2).repeat(1, 1, self.recon_part_M, 1)
        ...
        recon_to_ori_dist, ori_to_recon_dist = safe_chamfer_dist_call(expanded_recon_slot_pts, safe_transpose(ori_pts, -1, -2), self.chamfer_dist)
        recon_to_ori_dist = recon_to_ori_dist.mean(-1)
        ori_to_recon_dist =ori_to_recon_dist.mean(-1)

        tot_recon_loss = (recon_to_ori_dist + ori_to_recon_dist) * self.glb_recon_factor + (dist_slot_to_ori_pts + dist_ori_to_slot_pts) * self.slot_recon_factor
        ...
        selected_point_labels = ori_pts_slot_labels
        ...
        selected_attn = ori_pts_slot_one_hot_labels # one-hot labels actually
        ...
        out_feats['attn'] = ori_pts_slot_one_hot_labels.detach().cpu().numpy()

        if cur_iter == 0:
            self.attn_iter_0 = safe_transpose(ori_pts_slot_one_hot_labels, -1, -2)  # .cpu().numpy()
            self.attn_saved = safe_transpose(ori_pts_slot_one_hot_labels, -1, -2) # .contiguous().transpose(1, 2).contiguous().detach()
        elif cur_iter == 1:
            self.attn_iter_1 = safe_transpose(ori_pts_slot_one_hot_labels, -1, -2)  # .contiguous().transpose(1, 2).contiguous().detach()  # .cpu().numpy()
            self.attn_saved_1 = safe_transpose(ori_pts_slot_one_hot_labels, -1, -2) # .contiguous().transpose(1, 2).contiguous().detach()
        ...
        if tot_nll_orbit_selection_loss is not None:
            print(f"tot_nll_orbit_selection_loss: {tot_nll_orbit_selection_loss.item()}")
            tot_loss = tot_loss + 0.01 * tot_nll_orbit_selection_loss
