...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.log_fn = f"out_feats_weq_wrot_{self.global_rot}_rel_rot_factor_{self.rot_factor}_equi_{self.use_equi}_model_{self.model_type}_decoder_{self.decoder_type}_inv_attn_{self.inv_attn}_orbit_attn_{self.orbit_attn}_slot_iters_{self.slot_iters}_topk_{self.topk}_num_iters_{self.num_iters}_npts_{self.npoints}_perpart_npts_{self.part_pred_npoints}_bsz_{self.batch_size}_init_lr_{self.init_lr}"
    ...
    def forward_one_iter(self, x, pose, rlabel=None, cur_iter=0): # rotation label
        ...
        selected_recon_slot_points = batched_index_select(values=recon_slot_points, indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(2)
        ...
        sampled_recon_pts = expanded_recon_slot_points.contiguous().view(bz * (self.recon_part_M) * (self.num_slots), -1)[fps_idx, :].contiguous().view(bz, npoints, -1)
        ...
        tot_recon_loss = glb_dist_ori_to_recon + glb_dist_recon_to_ori + avg_dist_chamfer_recon_slot_ori
        ...
        transformed_ori_pts = torch.matmul(selected_inv_pred_R, ori_pts.unsqueeze(-1)).squeeze(-1)
    ...
    def forward(self, x, pose, rlabel=None, nn_inter=2):
        ...
        for i in range(nn_inter):
            cur_reconstructed_loss_orbit, attn, cur_estimated_pose = self.forward_one_iter(cur_transformed_points, cur_estimated_pose, rlabel=rlabel, cur_iter=i)
