...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        # so if we jsut use centralized x in

        # x = x - torch.mean(x, dim=-1, keepdim=True)
        # pose: bz x N x 4 x 4; gt_pose:
        # offset: bz x 1 x 3
        # offset = torch.mean(x, dim=-1, keepdim=True).squeeze(-2)
        # x = x - torch.mean(x, dim=-1, keepdim=True)
        ...
        # estimate global pose first and then use the rotation estimation to get rotation range of parts
        ...
        topk_anchor_idxes = None
        k = cur_kanchor

        if self.sel_mode is not None:
            k = 1
            topk_anchor_idxes = torch.tensor([self.sel_mode], dtype=torch.long).cuda().unsqueeze(0).unsqueeze(
                0).repeat(bz, self.num_slots, 1).contiguous()
