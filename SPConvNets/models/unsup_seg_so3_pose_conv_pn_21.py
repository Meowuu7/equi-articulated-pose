...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        ''' PointNet for feature aggregation '''
        ...
        self.axis_prediction = nn.Sequential(
        ...
        ''' Construct transformation prediction network for each slot '''
        ...
        ''' Initialize the transformation prediction network '''
        for zz in self.transformation_prediction:
        ...
        ''' Construct global transformation prediction network '''
        self.glb_transformation_prediction = nn.Sequential(
            nn.Conv2d(in_channels=self.encoded_feat_dim, out_channels=self.encoded_feat_dim // 2,
                      kernel_size=(1, 1),
                      stride=(1, 1), bias=True),
            nn.BatchNorm2d(num_features=self.encoded_feat_dim // 2),
        ...
        ''' Initialize global transformation prediction network '''
        for zz in self.glb_transformation_prediction:
            if isinstance(zz, nn.Sequential):
                for zzz in zz:
                    if isinstance(zzz, nn.Conv2d):
                        torch.nn.init.xavier_uniform_(zzz.weight)
        ...
        # pooling methid: 'pointnet' or 'max' or 'mean'
        # self.glb_trans_outblk_RT = SO3OutBlockRT(params['outblock'], norm=1, pooling_method='max', global_scalar=False, use_anchors=True, feat_mode_num=self.kanchor, num_heads=1)

        self.glb_trans_outblk_RT = SO3OutBlockR(params['outblock'], norm=1, pooling_method='max', pred_t=False, feat_mode_num=self.kanchor)
        ...
        ''' Network for whole shape reconstruction '''
        # self.whole_shp_recon_net = nn.Sequential( # 1024
        #     nn.Conv2d(in_channels=self.encoded_feat_dim, out_channels=256, kernel_size=(1, 1),
        #               stride=(1, 1), bias=True),
        #     # nn.BatchNorm2d(num_features=1024),
        ...
        #     # nn.BatchNorm2d(num_features=256),
        #     nn.LeakyReLU(inplace=True),
        #     nn.Conv2d(in_channels=256, out_channels=self.npoints * 3, # reconstruct the whole shape
        #               kernel_size=(1, 1), stride=(1, 1), bias=True),
        #     nn.Sigmoid()
        # )

        ''' Initialize the part reconstruction network '''
        # for zz in self.whole_shp_recon_net:
        #     if isinstance(zz, nn.Sequential):
        #         for zzz in zz:
        #             if isinstance(zzz, nn.Conv2d):
        #                 torch.nn.init.xavier_uniform_(zzz.weight)
        ...
        self.whole_shp_recon_net = DecoderFC([256, 256], self.encoded_feat_dim, self.npoints, None)
        ...
        self.slot_pair_mult_R_queue = torch.zeros((self.queue_tot_len, 16, 3, 3), dtype=torch.float, requires_grad=False).cuda()
    ...
    def update_slot_pair_axis_prior(self, axises, factor=0.9):
        ...
        axises[dot_axises_with_prior.unsqueeze(-1).repeat(1, 1, 3).contiguous() < 0.] = axises[dot_axises_with_prior.unsqueeze(-1).repeat(1, 1, 3).contiguous() < 0.] * (-1.0)
    ...
    def select_anchors_via_previous_consistency(self, dot_anchor_part_rots_anchors):
        ...
        print(f"Current queue size: {cur_queue.size()}, queue len: {self.queue_len}, queue total len: {self.queue_tot_len}")
        dot_rots_cur_queue = torch.matmul(dot_anchor_part_rots_anchors.unsqueeze(1), cur_queue.view(1, self.queue_len, self.num_slots, 1, 1, 3, 3).contiguous())
        dot_ax_x, dot_ax_y, dot_ax_z = dot_rots_cur_queue[..., 2, 1] - dot_rots_cur_queue[..., 1, 2], dot_rots_cur_queue[..., 0, 2] - dot_rots_cur_queue[..., 2, 0], dot_rots_cur_queue[..., 1, 0] - dot_rots_cur_queue[..., 0, 1]
    ...
    def update_slot_pair_mult_R_queue(self, selected_mult_R):
        ...
        if self.queue_st + bz > self.queue_tot_len:
            self.slot_pair_mult_R_queue.data[self.queue_st: ] = selected_mult_R[: self.queue_tot_len - self.queue_st].detach()
            self.slot_pair_mult_R_queue.data[: bz - self.queue_tot_len + self.queue_st] = selected_mult_R[self.queue_tot_len - self.queue_st: ].detach()
        else:
    ...
    def select_slot_orbits_bak(self, slot_recon_loss, slot_pred_rots):
        ...
        if self.use_axis_queue == 1:
            if self.queue_len < 2:
                ...
                selected_slot_pred_rots = batched_index_select(values=slot_pred_rots, indices=selected_slot_oribt.unsqueeze(-1), dim=2)
                ...
                anchor_part_others_rel_rots = torch.matmul(selected_slot_pred_rots[:, 0, :, :].unsqueeze(1).contiguous().transpose(-1, -2).contiguous(), selected_slot_pred_rots)
        ...
        ax_x, ax_y, ax_z = dot_anchor_part_rots_anchors[..., 2, 1] - dot_anchor_part_rots_anchors[..., 1, 2], dot_anchor_part_rots_anchors[..., 0, 2] - dot_anchor_part_rots_anchors[..., 2, 0], dot_anchor_part_rots_anchors[..., 1, 0] - dot_anchor_part_rots_anchors[..., 0, 1]
        ...
        anchor_part_anchors_axises = anchor_part_anchors_axises / torch.clamp(torch.norm(anchor_part_anchors_axises, dim=-1, keepdim=True, p=2), min=1e-8)

        # dot_anchor_part_anchors_axises_prior: bz x n_s x na x na
        dot_anchor_part_anchors_axises_prior = torch.sum(anchor_part_anchors_axises * self.axis_prior_slot_pairs.view(1, self.num_slots, 1, 1, 3).contiguous(), dim=-1)
        ...
        if self.use_axis_queue == 1:
            ...
            selected_dots, selected_anchors = self.select_anchors_via_previous_consistency(dot_anchor_part_rots_anchors=dot_anchor_part_rots_anchors)
        else:
            ''' Single shape selection '''
            # use queue for selection
            selected_dots, selected_anchors = torch.topk(dot_anchor_part_anchors_axises_prior, k=5, largest=True, dim=-1)
        ...
        selected_dot_anchor_part_rots_anchors = batched_index_select(values=dot_anchor_part_rots_anchors, indices=selected_anchors.long(), dim=3)

        # select angles
        selected_dot_anchor_part_rots_anchors_angles = selected_dot_anchor_part_rots_anchors[..., 0, 0] + selected_dot_anchor_part_rots_anchors[..., 1, 1] + selected_dot_anchor_part_rots_anchors[..., 2, 2]
        ...
        selected_angles, selected_anchors_angles = torch.topk(selected_dot_anchor_part_rots_anchors_angles, k=3, largest=True, dim=-1)
        ...
        res_selected_loss = batched_index_select(values=slot_recon_loss, indices=res_selected_anchors.unsqueeze(-1), dim=2).squeeze(2)
        anchor_part_selected_anchor = res_selected_anchors[:, 0]
        # anchor_part_anchors_axises: bz x n_s x na x 3
        anchor_part_anchors_axises = batched_index_select(safe_transpose(anchor_part_anchors_axises, 1, 2), indices=anchor_part_selected_anchor.unsqueeze(-1), dim=1).squeeze(1)
        # anchor_part_anchors_dot_rotations: bz x n_s x na x 3 x 3
        anchor_part_anchors_dot_rotations = batched_index_select(safe_transpose(dot_anchor_part_rots_anchors, 1, 2), indices=anchor_part_selected_anchor.unsqueeze(-1), dim=1).squeeze(1)
        # bz x n_s x na x 3 --> bz x na x n_s x 3 --> bz x n_s x 3
        # dot product between the first slot and remaining slots
        selected_dots = batched_index_select(values=safe_transpose(selected_dots, 1, 2), indices=anchor_part_selected_anchor.unsqueeze(-1), dim=1).squeeze(1)
        ...
        anchor_part_anchors_axises = batched_index_select(values=anchor_part_anchors_axises, indices=res_selected_anchors.unsqueeze(-1), dim=2).squeeze(2)
        # anchor_part_anchors_dot_rotations: bz x n_s x 3 x 3
        anchor_part_anchors_dot_rotations = batched_index_select(values=anchor_part_anchors_dot_rotations, indices=res_selected_anchors.unsqueeze(-1), dim=2).squeeze(2)
        ...
        if self.run_mode == "train":
            ...
            if self.use_axis_queue == 1:
                ...
                np.save(f"slot_pair_mult_R_queue_{self.local_rank}.npy", self.slot_pair_mult_R_queue.data.detach().cpu().numpy())
                ''' Save slot prior mult R queue --- for multi shape selection '''
                pass
            else:
            ...
            for i_a in range(self.kanchor):
                ...
                for i_perm in range(nn_permute):
                    ...
                    for i_other_part in range(self.num_slots - 1):
                        ...
                        cosine_sum_parts += dot_anchor_part_anchors_axises_prior[i_bz, 1 + i_other_part, i_a, real_orbit_idx].item() * self.slot_weights[i_bz, 1 + i_other_part].item()
                        zz_i_perm = zz_i_perm // (self.num_slots - 1)
                        orbit_cmb.append(real_orbit_idx)
    ...
    def from_rotation_mtx_to_axis(self, rots):
        ...
        for i in range(exp_rots.size(0)):
            ...
            if abs(trace - 1.0) < 1e-8:
                ...
                ax_x, ax_y, ax_z = cur_rot[2, 1] - cur_rot[1, 2], cur_rot[0, 2] - cur_rot[2, 0], cur_rot[1, 0] - cur_rot[0, 1]
                axes = torch.tensor([ax_x, ax_y, ax_z], dtype=torch.float).cuda()
                axes = axes / (2.0 * sine)
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        # centralize points
        # for further
        # the first iteration --- global rotation and translation factorization
        x = x if cur_iter == 0 else self.glb_trans_pts
        if cur_iter == 0:
            # cent_trans:bz x 3
            self.pure_original_x = x.clone()
            cent_trans = torch.mean(x, dim=-1)
        ...
        # if x.size(-1) > 3:
        #     x = safe_transpose(x, -1, -2)
        # x = Mso3.preprocess_input(x, self.kanchor, False)
        # print(f"before encoding, x.feats: {x.feats.size()}")
        cur_backbone = self.glb_backbone if cur_iter == 0 else self.backbone
        for block_i, block in enumerate(cur_backbone):
            # x = block(x, seg=seg)
            x = block(x)

        # print(f"After encoding, x.feats: {x.feats.size()}")



        if cur_iter == 0:
            # x_out, orbit_attn_weights, var_out_feats = self.glb_outblock(x)
            # if len(orbit_attn_weights.size()) == 1:
            #     orbit_attn_weights = orbit_attn_weights.unsqueeze(0)

            x_out = self.glb_outblock(x)

            # print(f"manifold_embed: {x_out.size()}")

            # glb_recon_pts: bz x (npoints * 3) x 1 x 1
            # glb_recon_pts = self.whole_shp_recon_net(x_out.unsqueeze(-1).unsqueeze(-1))
            glb_recon_pts = self.whole_shp_recon_net(x_out)
            # print("here", torch.mean(glb_recon_pts).item())
            glb_recon_pts = glb_recon_pts - 0.5

            # glb_recon_pts: bz x (npoints * 3)
            glb_recon_pts = glb_recon_pts.squeeze(-1).squeeze(-1)
            glb_recon_pts = glb_recon_pts.contiguous().view(bz, self.npoints, 3).contiguous()

            output_RT = self.glb_trans_outblk_RT(x, self.anchors)
            glb_res_R = output_RT['R'] # bz x 4 x na
            # glb_res_T = output_RT['T'] # bz x 3 x na
            # print("glb_res_T,", glb_res_T.size())

            # glb_trans_prediction = self.glb_transformation_prediction(var_out_feats.unsqueeze(-1))
            # # glb_trans_prediction: bz x 7 x na
            # glb_trans_prediction = glb_trans_prediction.squeeze(-1)
            # glb_res_R, glb_res_T = glb_trans_prediction[:, :4, :], glb_trans_prediction[:, 4:, :]

            # and only use rotation for regression

            # glb_res_T: bz x 3 x na --> bz x na x 3
            # glb_res_T = safe_transpose(glb_res_T, -1, -2)

            glb_regu_quat_loss = None
            # glb_pred_qw, glb_pred_qxyz = torch.split(safe_transpose(glb_res_R, 1, 2), [1, 3], dim=-1)
            # theta_max = torch.Tensor([36 / 180 * np.pi]).cuda()  # 36 * 5 --> 180
            # glb_pred_qw = torch.cos(theta_max) + (1 - torch.cos(theta_max)) * F.sigmoid(glb_pred_qw)
            # glb_constrained_quat = torch.cat([glb_pred_qw, glb_pred_qxyz], dim=-1)
            # glb_res_R = glb_constrained_quat
            # # todo: add quat loss to total loss
            # glb_regu_quat_loss = torch.mean(torch.pow(torch.norm(glb_constrained_quat, dim=-1) - 1, 2))

            glb_res_R = compute_rotation_matrix_from_quaternion(safe_transpose(glb_res_R, 1, 2).view(bz * self.kanchor, -1)).contiguous().view(bz, self.kanchor, 3, 3)
            # self.anchors: na x 3 x 3; glb_res_R: bz x na x 3 x 3 ---> glb_R: bz x na x 3 x 3
            glb_R = torch.matmul(self.anchors.unsqueeze(0), glb_res_R)

            # glb_res_T = torch.matmul(self.anchors.unsqueeze(0), glb_res_T.unsqueeze(-1)).squeeze(-1)

            # ori_pts: bz x 3 x N -> bz x 1 x 3 x N ---> bz x na x 3 x N ---> bz x na x N x 3 (transformed_pts)
            ''' Use predicted r and t for further transformation '''
            # glb_res_T: bz x na x 3 ---> predicted residual translation
            # transformed_pts: bz x na x N x 3
            transformed_pts = safe_transpose(torch.matmul(glb_R, safe_transpose(glb_recon_pts, -1, -2).unsqueeze(1)), -1, -2) # + glb_res_T.unsqueeze(2)
            # ori_pts: bz x 3 x N --> bz x N x 3
            transposed_ori_pts = safe_transpose(ori_pts, -1, -2)
            transposed_ori_pts_tiled = transposed_ori_pts.unsqueeze(1).repeat(1, self.kanchor, 1, 1)
            # cd_trans_ori: bz x na x N x N
            # cd_trans_ori = torch.sum((transformed_pts.unsqueeze(3) - transposed_ori_pts.unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
            # # cd_trans_ori = torch.sum(torch.abs(transformed_pts.unsqueeze(3) - transposed_ori_pts.unsqueeze(1).unsqueeze(1)) , dim=-1)
            # cd_trans_to_ori, _ = torch.min(cd_trans_ori, dim=-1)
            # cd_ori_to_trans, _ = torch.min(cd_trans_ori, dim=-2)

            cd_trans_to_ori, cd_ori_to_trans = safe_chamfer_dist_call(transformed_pts.contiguous().view(bz * self.kanchor, self.npoints, 3), transposed_ori_pts_tiled.contiguous().view(bz * self.kanchor, self.npoints, 3), self.chamfer_dist)

            cd_trans_to_ori = cd_trans_to_ori.contiguous().view(bz, self.kanchor, self.npoints)
            cd_ori_to_trans = cd_ori_to_trans.contiguous().view(bz, self.kanchor, self.npoints)

            # cd_trans_ori: bz x na
            cd_trans_ori = cd_trans_to_ori.mean(-1) + cd_ori_to_trans.mean(-1)
            # minn_cd: bz; glb_orbit: bz
            minn_cd, glb_orbit = torch.min(cd_trans_ori, dim=-1)
            # print(f"glb_regu_quat_loss: {glb_regu_quat_loss.item()}")
            # tot_loss = minn_cd.mean() + glb_regu_quat_loss * 10.0

            tot_loss = minn_cd.mean()
            # tot_loss = minn_cd.mean() + glb_regu_quat_loss * 0.1
            if glb_regu_quat_loss is not None:
                tot_loss = tot_loss + 0.1 * glb_regu_quat_loss

            # sel_glb_R: bz x 3 x 3
            sel_glb_R = batched_index_select(glb_R, glb_orbit.unsqueeze(-1), dim=1).squeeze(1)
            # sel_glb_T: bz x 3
            # sel_glb_T = batched_index_select(glb_res_T, glb_orbit.unsqueeze(-1), dim=1).squeeze(1)
            # sel_transformed_reconstructed_pts: bz x N x 3
            sel_transformed_reconstructed_pts = batched_index_select(values=transformed_pts, indices=glb_orbit.unsqueeze(1), dim=1).squeeze(1)

            # glb_trans_pts: bz x 3 x N

            # glb_trans_pts = torch.matmul(safe_transpose(sel_glb_R, -1, -2), safe_transpose(safe_transpose(ori_pts, -1, -2) - sel_glb_T.unsqueeze(1), -1, -2))
            glb_trans_pts = torch.matmul(safe_transpose(sel_glb_R, -1, -2), safe_transpose(safe_transpose(ori_pts, -1, -2), -1, -2))

            self.glb_trans_pts = glb_trans_pts # .detach().cpu()
            # self.recon_pts: bz x N x 3
            self.recon_pts = sel_transformed_reconstructed_pts.detach().cpu() # .detach().cpu().numpy()
            # self.ori_recon_pts: bz x N x 3
            self.ori_recon_pts = glb_recon_pts.detach().cpu() # .detach().cpu().numpy()

            out_feats = {}
            out_feats['glb_inv_transformed_pts'] = self.glb_trans_pts.detach().cpu().numpy()
            out_feats['transformed_recon_pts'] = self.recon_pts.detach().cpu().numpy()
            out_feats['ori_recon_pts'] = self.ori_recon_pts.detach().cpu().numpy()
            out_feats['ori_pts'] = ori_pts.detach().cpu().numpy()

            np.save(self.log_fn + f"_n_iter_{cur_iter}.npy", out_feats)

            self.out_feats = out_feats

            # glb_pred_trans = sel_glb_T + cent_trans
            # self.glb_pose: bz x 3 x 4
            # print(sel_glb_R.size(), glb_pred_trans.size())
            # self.glb_pose = torch.cat([sel_glb_R, glb_pred_trans.unsqueeze(-1)], dim=-1)
            self.glb_pose = sel_glb_R
            return tot_loss
        else:
            ...
            # self.hard_slot_indicatorss = (torch.sum(hard_one_hot_labels_slot, dim=-1) > 0.5).float()
            ...
            self.slot_weights = hard_slot_weights
            ...
            pred_R = compute_rotation_matrix_from_quaternion(pred_R.contiguous().permute(0, 2, 3, 1).contiguous().view(-1, 4)).contiguous().view(bz, self.num_slots, self.kanchor, 3, 3)
            ...
            pred_T = pred_T.contiguous().permute(0, 2, 3, 1).contiguous()
            ...
            # dist_chamfer_recon_slot = dist_chamfer_recon_slot * self.hard_slot_indicatorss
            # bz x n_s x na
            ...
            # dist_chamfer_recon_slot_ori: bz x ns; selected_slot_oribt: bz x ns
            ...
            expanded_ori_recon_slot_points = (recon_slot_points * hard_slot_indicator.unsqueeze(-1).unsqueeze(-1)).contiguous().view(bz, self.num_slots * self.recon_part_M, 3)
            ...
            fps_idx_ori = farthest_point_sampling(expanded_ori_recon_slot_points, npoints)
            sampled_ori_recon_pts = expanded_ori_recon_slot_points.contiguous().view(bz * (self.recon_part_M) * (self.num_slots),
                                                                             -1)[fps_idx_ori, :].contiguous().view(bz, npoints,
                                                                                                               -1)
            ...
            # tot_recon_loss = glb_recon_factor * (glb_dist_ori_to_recon + glb_dist_recon_to_ori + glb_dist_recon_to_ori_ori + glb_dist_ori_to_recon_ori) + slot_recon_factor * avg_dist_chamfer_recon_slot_ori
            ...
            #
            pure_ori_x = safe_transpose(self.pure_original_x, -1, -2)
            ...
            selected_pred_R = torch.matmul(glb_rot.unsqueeze(1), selected_pred_R)
            ...
            selected_pred_T = batched_index_select(values=pred_T, indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(2)
            print(glb_rot.size(), selected_pred_T.size())
            selected_pred_T = torch.matmul(glb_rot.unsqueeze(1), selected_pred_T.unsqueeze(-1)).squeeze(-1) # + glb_trans # .unsqueeze(1)
            # print("selected_pred_T", selected_pred_T.size(), glb_trans.size())
            selected_pred_T = selected_pred_T + glb_trans
            selected_ori_pred_T = batched_index_select(values=ori_pred_T, indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(2)
            selected_ori_pred_T = torch.matmul(glb_rot.unsqueeze(1), selected_ori_pred_T.unsqueeze(-1)).squeeze(
                -1) + glb_trans # .unsqueeze(1)
            ...
            # Remember avg_selected_dots; size: bz x n_s;
            try:
                out_feats['avg_selected_dots'] = self.avg_selected_dots.detach().cpu().numpy()
            except:
                pass
            ...
            transformed_ori_pts = torch.matmul(selected_inv_pred_R, (pure_ori_x - selected_pred_T).unsqueeze(-1)).squeeze(-1)
            ...
            if gt_pose is not None:
                ...
                gt_transformed_ori_pts = torch.matmul(gt_inv_R, (pure_ori_x - gt_T).unsqueeze(-1)).squeeze(-1)
            ...
            print(f"contrained_quat_loss: {regu_quat_loss.item()}")
            # tot_loss = tot_loss + regu_quat_loss * 10.0  + glb_regu_quat_loss * 10.0 + oribt_selection_loss
            # tot_loss = tot_loss # + glb_regu_quat_loss * 10.0 # + oribt_selection_loss
            tot_loss = tot_loss + 10.0 * regu_quat_loss
    ...
    def forward(self, x, pose, ori_pc=None, rlabel=None, nn_inter=2, pose_segs=None, canon_pc=None):
        ...
        tot_loss = self.forward_one_iter(cur_transformed_points, cur_estimated_pose, ori_pc=ori_pc, rlabel=rlabel, cur_iter=0, gt_pose=pose, gt_pose_segs=pose_segs, canon_pc=canon_pc, selected_pts_orbit=cur_selected_pts_orbit)
