...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        ''' Construct category-level transformation prediction network '''
        # self.category_transformation_prediction = nn.ModuleList()
        # # Current: use the invariant feature to predict category-level transformations
        # # Then use the variant feature to predict instance-level transformation --- add a pointnet or just use
        # for i_s in range(self.num_slots):
        #     self.category_transformation_prediction.append(
        #         nn.Sequential(
        #             nn.Conv2d(in_channels=self.encoded_feat_dim, out_channels=self.encoded_feat_dim // 2,
        #                       kernel_size=(1, 1),
        #                       stride=(1, 1), bias=True),
        ...
        ''' Construct feature attention weights calculation network '''
        self.attention_weights_net = nn.ModuleList()
        for i_s in range(self.num_slots):
            self.attention_weights_net.append(
                nn.Sequential(
                    # nn.Conv2d(in_channels=self.encoded_feat_dim, out_channels=self.encoded_feat_dim // 2, kernel_size=(1, 1),
                    #           stride=(1, 1), bias=True),
                    # nn.BatchNorm2d(num_features=self.encoded_feat_dim // 2),
                    # nn.LeakyReLU(),
                    nn.Conv2d(in_channels=self.encoded_feat_dim, out_channels=1, kernel_size=(1, 1),
                              stride=(1, 1), bias=True)
                )
            )
        ''' Initialize the feature attention weights calculation network '''
        for zz in self.attention_weights_net:
        ...
        # self.slot_prior_rep = nn.ParameterList()
        # for i_s in range(self.num_slots):
        #     cur_slot_prior = torch.zeros((self.encoded_feat_dim), dtype=torch.float, requires_grad=False)
        #     cur_slot_prior = nn.Parameter(cur_slot_prior, requires_grad=False)
        #     self.slot_prior_rep.append(cur_slot_prior)
        ...
        ''' Context distribution for each slot's points '''
        # self.ctx_dist_prior = nn.ParameterList()
        # for i_s in range(self.num_slots):
            # cur_ctx_dist = nn.Parameter(torch.FloatTensor(self.recon_part_M, self.num_slots + 1, ), requires_grad=False)
            # self.ctx_dist_prior.append(cur_ctx_dist)
        self.ctx_dist_prior = torch.zeros((self.num_slots, self.recon_part_M, self.num_slots + 1), dtype=torch.float32, requires_grad=False).cuda()
        self.has_ctx_dist = False
        ''' Context distribution for each slot's points '''
    ...
    def calculate_ctx_dist(self, part_recon_M, ori_pts, pred_labels, r=0.2, k=32): # common radius
        # part_recon_M: bz x n_a x n_s x M x 3; ori_pts: bz x N x 3
        # pred_labels: bz x N
        # dist_recon_ori: bz x n_a x n_s x M x N
        dist_recon_ori = torch.sum((part_recon_M.unsqueeze(-2) - ori_pts.unsqueeze(1).unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
        # dist_recon_ori = -1. * dist_recon_ori
        # minn_k_dist: bz x n_a x n_s x M x k; minn_k_idxes: bz x n_a x n_s x M x k
        minn_k_dist, minn_k_idxes = torch.topk(dist_recon_ori, dim=-1, largest=False, k=k)
        # minn_k_labels: bz x n_a x n_s x M x k
        minn_k_labels = batched_index_select(values=pred_labels, indices=minn_k_idxes, dim=1)
        # set to none value
        minn_k_labels[minn_k_dist > r ** 2] = self.num_slots
        # minn_k_one_hot_labels: bz x n_a x n_s x M x k x (n_slots + 1)
        minn_k_one_hot_labels = torch.eye(self.num_slots + 1, dtype=torch.float).cuda()[minn_k_labels]
        # dist_neis: bz x n_a x n_s x M x (n_slots + 1)
        dist_neis = torch.sum(minn_k_one_hot_labels, dim=-2) / float(k)
        return dist_neis

    def get_slot_pts_dist_neis(self):
        if self.has_ctx_dist == False:
            return None
        else:
            dist_neis = []
            for i_s in range(self.num_slots):
                cur_dist_nei = self.ctx_dist_prior[i_s]
                dist_neis.append(cur_dist_nei.unsqueeze(0))
            # dist_neis: n_s x M x (num_slots + 1)
            dist_neis = torch.cat(dist_neis, dim=0)
            return dist_neis

    def calculate_matching_prob(self, mem_dist_neis, curr_dist_neis):
        # mem_dist_neis: n_s x M x (num_slots + 1)
        # curr_dist_neis: bz x n_a x n_s x M x (n_slots + 1)
        bz = curr_dist_neis.size(0)
        exp_mem_dist_neis = mem_dist_neis.unsqueeze(0).unsqueeze(0).repeat(bz, self.kanchor, 1, 1, 1)
        # dot_product: bz x n_a x n_s x M
        dot_product = torch.sum(exp_mem_dist_neis * curr_dist_neis, dim=-1)
        # sum over M --- get the matching score from this reconstruction frame to original pts at the xxx anchor
        return dot_product

    def update_slot_pts_dist_neis(self, curr_selected_dist_neis, factor=0.60):
        # curr_selected_dist_neis: bz x n_s x M x (n_slots + 1)
        # dist_neis:
        # mean_curr_selected_dist_neis: n_s x M x (n_slots + 1)
        # update context distribution prior

        mean_curr_selected_dist_neis = curr_selected_dist_neis.mean(dim=0)
        if self.has_ctx_dist is False:
            factor = 0.0
        #
        for i_s in range(self.num_slots):
            self.ctx_dist_prior[i_s] = self.ctx_dist_prior[i_s] * factor + (1. - factor) * mean_curr_selected_dist_neis[i_s]
            #
            self.ctx_dist_prior[i_s] = self.ctx_dist_prior[i_s] / torch.clamp(torch.sum(self.ctx_dist_prior[i_s], dim=-1, keepdim=True), min=1e-8)
        self.has_ctx_dist = True
        return
    ...
    def apply_feature_attention_weights_calculation_network(self, slot_feats):
        # slot_weights: bz x dim x n_s x na
        selected_orbit = []
        for i_s in range(self.num_slots):
            # cur_slot_feats: bz x dim x na
            cur_slot_feats = slot_feats[:, :, i_s, :]
            # cur_slot_orbit_weights: bz x 1 x na x 1
            cur_slot_orbit_weights = self.attention_weights_net[i_s](cur_slot_feats.unsqueeze(-1))
            # cur_slot_orbit_weights: bz x na
            cur_slot_orbit_weights = cur_slot_orbit_weights.squeeze(-1).squeeze(1)
            # maxx_slot_orbit_weight: bz; selected_slot_orbit: bz
            maxx_slot_orbit_weight, selected_slot_orbit = torch.max(cur_slot_orbit_weights, dim=1)
            selected_orbit.append(selected_slot_orbit.unsqueeze(1))
        selected_orbit = torch.cat(selected_orbit, dim=1)
        return selected_orbit

    def apply_category_transformation_network(self, inv_slot_feats):
        # ivn_slot_feats: bz x dim x n_s
        category_transformation_slot = []
        for i_s in range(self.num_slots):
            # cur_slot_inv_feats: bz x dim
            cur_slot_inv_feats = inv_slot_feats[:, :, i_s]
            cur_slot_category_transformation = self.category_transformation_prediction[i_s](cur_slot_inv_feats.unsqueeze(-1).unsqueeze(-1))
            # bz x 7
            cur_slot_category_transformation = cur_slot_category_transformation.unsqueeze(-1).unsqueeze(-1)
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        ''' Use feature itself to select the orbit '''
        selected_orbit = self.apply_feature_attention_weights_calculation_network(variant_feats_slot)
        ...
        ''' Further transform the predicted translation '''
        ...
        ''' Select translation and rotation '''
        # pred_T: bz x n_s x 3
        pred_T = batched_index_select(values=pred_T, indices=selected_orbit.unsqueeze(-1), dim=2).squeeze(2)
        # pred_R: bz x n_s x 3 x 3
        pred_R = batched_index_select(values=pred_R, indices=selected_orbit.unsqueeze(-1), dim=2).squeeze(2)

        ori_pred_T = pred_T.detach().clone()

        ''' From predicted rotation matrix and translation matrix to transformed points '''
        # transformed_slot_pts = torch.matmul(pred_R, recon_slot_points.unsqueeze(2).contiguous().transpose(-1,
        #                                                                                                   -2)).contiguous().transpose(
        #     -1, -2) + pred_T.unsqueeze(-2)
        # recon_slot_points: bz x n_s x M x 3 --> transformed_slot_pts: bz x n_s x M x 3

        transformed_slot_pts = safe_transpose(torch.matmul(pred_R, safe_transpose(recon_slot_points, -1, -2)), -1, -2) + pred_T.unsqueeze(-2)

        # 1 x 1 x na x 3 x 3
        anchors_transformed_pts = torch.matmul(self.anchors.unsqueeze(0).unsqueeze(0), safe_transpose(transformed_slot_pts, -1, -2).unsqueeze(2), )
        # anchors_transformed_pts: bz x n_s x na x M x 3
        anchors_transformed_pts = safe_transpose(anchors_transformed_pts, -1, -2)
        anchors_transformed_pts = safe_transpose(anchors_transformed_pts, 1, 2)
        ...
        expanded_recon_slot_points = (anchors_transformed_pts * hard_slot_indicator.unsqueeze(1).unsqueeze(-1).unsqueeze(-1)).contiguous().view(bz * self.kanchor, self.num_slots * self.recon_part_M, 3)

        # sampled_recon_pts: (bz * na) x npoints x 3
        fps_idx = farthest_point_sampling(expanded_recon_slot_points, npoints)
        # sampled_recon_pts: bz x na x N x 3
        sampled_recon_pts = expanded_recon_slot_points.contiguous().view(bz * self.kanchor * (self.recon_part_M) * (self.num_slots), -1)[fps_idx, :].contiguous().view(bz, self.kanchor, npoints, -1).contiguous()

        ''' Sampled-Ori Chamfer distance hand-crafted calculation version '''
        # bz x na x npoints x npoints
        glb_dist_recon_ww_ori = torch.sum((sampled_recon_pts.unsqueeze(2) - ori_pts.unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
        ...
        # bz x na
        glb_recon_dist = glb_dist_recon_to_ori + glb_dist_ori_to_recon
        # bz; bz
        minn_glb_recon_dist, selected_glb_orbit = torch.min(glb_recon_dist, dim=-1)

        selected_glb_orbit_slot = selected_glb_orbit.unsqueeze(-1).repeat(1, self.num_slots).long()
        selected_anchored_pts = batched_index_select(values=safe_transpose(anchors_transformed_pts, 1, 2), indices=selected_glb_orbit_slot.unsqueeze(-1), dim=2)
        selected_anchored_pts = selected_anchored_pts.squeeze(2)

        # selected_anchored_pts: bz x n_s x M x 3
        # ori_pts: bz x N x 3
        # dist_recon_ori_slot: bz x n_s x M x N
        dist_recon_ori_slot = torch.sum((selected_anchored_pts.unsqueeze(3) - ori_pts.unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
        # hard_one_hot_labels: bz x n_s x N
        ...
        # dist_recon_to_ori: bz x n_s x M
        dist_recon_to_ori, _ = torch.min(dist_recon_ori_slot, dim=-1)
        dist_recon_to_ori[dist_recon_to_ori > 100.] = 0.
        # dist_ori_to_recon: bz x n_s x N
        dist_ori_to_recon, _ = torch.min(dist_recon_ori_slot, dim=-2)
        dist_ori_to_recon[dist_ori_to_recon > 100.] = 0.
        dist_recon_to_ori = torch.sum(dist_recon_to_ori * hard_slot_indicator.unsqueeze(-1), dim=-1).mean(dim=-1)
        dist_ori_to_recon = torch.sum(dist_ori_to_recon * hard_slot_indicator.unsqueeze(-1), dim=-1).mean(dim=-1)

        slot_recon_loss = dist_recon_to_ori + dist_ori_to_recon

        # for i_bz in range(bz):
        #     for i_s in range(self.num_slots):

        #
        selected_glb_anchor = batched_index_select(values=self.anchors, indices=selected_glb_orbit.unsqueeze(1), dim=0)
        # selected_glb_anchor: bz x 3 x 3
        selected_glb_anchor = selected_glb_anchor.squeeze(1)

        # pred_R: bz x n_s x 3 x 3
        pred_R = torch.matmul(selected_glb_anchor.unsqueeze(1), pred_R)
        # pred_T: bz x n_s x 3
        pred_T = torch.matmul(selected_glb_anchor.unsqueeze(1), pred_T.unsqueeze(-1)).squeeze(-1)
        ori_pred_T = torch.matmul(selected_glb_anchor.unsqueeze(1), ori_pred_T.unsqueeze(-1)).squeeze(-1)

        tot_recon_loss = minn_glb_recon_dist * self.glb_recon_factor + slot_recon_loss * self.slot_recon_factor
        ...
        # selected_recon_slot_pts = transformed_slot_pts
        # selected_anchored_pts: bz x n_s x M x 3
        selected_recon_slot_pts = selected_anchored_pts
        ...
        selected_pred_T = pred_T
        selected_ori_pred_T = ori_pred_T
