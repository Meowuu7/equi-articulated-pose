...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        # self.pred_t = False
        self.pred_t = True
        self.whether_pred_R = True if self.shape_type != "drawer" else False
        for i_s in range(self.num_slots):
            ...
            if not self.pred_t:
                ...
                self.slot_trans_outblk_RT.append(
                    SO3OutBlockRTWithMask(params['outblock'], norm=1, pooling_method='max',
                                          ...
                                          # global scalar?
                                          use_anchors=False,
                                          feat_mode_num=self.kanchor, num_heads=1, pred_R=self.whether_pred_R)
                )
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        # print(f"after backbone --- encoded features: {x.feats.size()}")
        ...
        for i_s in range(self.num_slots):
            ...
            for i_bz in range(bz):
                ...
                # else:
                #     cur_bz_cur_slot_inv_feats = torch.zeros((1, x.feats.size(1)), dtype=torch.float).cuda()
                #     cur_bz_cur_slot_orbit_confidence = torch.zeros((1, x.feats.size(-1)), dtype=torch.float).cuda()
                #
                #     cur_slot_inv_feats.append(cur_bz_cur_slot_inv_feats)
                #     cur_slot_orbit_confidence.append(cur_bz_cur_slot_orbit_confidence)
                #
                #     cur_bz_cur_slot_R = torch.ones((1, 4, self.kanchor), dtype=torch.float).cuda()
                #     cur_bz_cur_slot_T = torch.zeros((1, 3, self.kanchor), dtype=torch.float).cuda()
                #
                #     print("here 1")
                #     cur_slot_R.append(cur_bz_cur_slot_R)
        ...
        if self.whether_pred_R:
            ...
            slot_R = torch.eye(3, dtype=torch.float32).cuda().contiguous().unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(bz, self.num_slots, self.kanchor, 1, 1)
        # print("here 3")
