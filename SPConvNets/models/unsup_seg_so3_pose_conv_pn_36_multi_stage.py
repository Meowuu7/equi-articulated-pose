...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.feat_partition = params['general']['feat_partition']  # --rel-for-points
        ...
        ori_outblock_mlp = params['outblock']['mlp'][-1]
        if self.feat_partition == 1:
            params['outblock']['mlp'][-1] = ori_outblock_mlp * 2
        ...
        if self.feat_partition == 1:
            params['outblock']['mlp'][-1] = ori_outblock_mlp
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if self.stage == 0:
            ...
            whl_shp_inv_feats, whl_shp_orbit_confidence = self.whole_shp_outblock(x,
                                                                                  mask=None)  # Get whole shape invariant feature
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    ''' Get part coordinates and part shapes from proposed parts '''
                    ...
                    ''' Use coordinates and features from all points for feature computing '''
                    cur_bz_cur_slot_xyz = safe_transpose(x.xyz, -1, -2)[i_bz].unsqueeze(0)
                    cur_bz_cur_slot_feats = safe_transpose(x.feats, 1, 2)[i_bz].unsqueeze(0)
                    pair_cur_bz_cur_slot_xyz = safe_transpose(x.xyz, -1, -2)[i_bz].unsqueeze(0)  #
                    pair_cur_bz_cur_slot_feats = safe_transpose(x.feats, 1, 2)[i_bz].unsqueeze(0)
                    ...
                    if self.pred_pv_equiv:
                        ...
                        cur_bz_cur_slot_abs_equiv_feats, cur_bz_cur_slot_abs_inv_feats, cur_bz_cur_slot_orbit_confidence = \
                            self.abs_slot_outblock[i_s](cur_bz_cur_slot_x, mask=None)
                    else:
                    ...
                    # cur_bz_cur_slot_inv_feats: bz x dim
                    ...
                    #### expanded pair current bz current slot invariant features for invariant feature prediction ####
                    ...
                    # if i_s == 0:
                    #     cur_bz_cur_slot_inv_feats = torch.cat([curbzcursloti])

                    if self.feat_partition:
                        per_slot_feat_dim = cur_bz_cur_slot_inv_feats.size(1) // self.num_slots
                        cur_bz_cur_slot_inv_feats = cur_bz_cur_slot_inv_feats[:, i_s * (per_slot_feat_dim): (i_s + 1) * per_slot_feat_dim]
                        cur_bz_cur_slot_abs_inv_feats = cur_bz_cur_slot_abs_inv_feats[:, i_s * (per_slot_feat_dim): (i_s + 1) * per_slot_feat_dim]
                    ...
                    if use_whl_shp_inv_feats:
                        ...
                        cur_slot_inv_feats.append(cur_bz_cur_slot_inv_feats) # inv feature from a single slot
                        cur_pair_slot_inv_feats.append(pair_cur_bz_cur_slot_inv_feats) # from two slots
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    if i_s == cur_bz_cur_minn_seg_label:  #
                        ...
                        # cur_slot_trans = -1.0 * torch.matmul(cur_bz_cur_slot_R, cur_slot_pv_point.unsqueeze(-1)) + (
                        #             cur_slot_pv_point + cur_slot_central_point).unsqueeze(-1)  #
                        cur_slot_trans = cur_slot_trans.squeeze(-1).unsqueeze(0).unsqueeze(0)
                        ''' V3 -- For base part: Use both pvp and central p for translation prediction '''
                    else:
                        if self.recon_prior == 6 or self.recon_prior == 7 or self.recon_prior == 8 or self.recon_prior == 9:
            ...
            # minn_dist_ori_to_recon: ori_to_recon?
            ...
            orbit_slot_dist_ori_recon = minn_dist_recon_to_ori # .clone()
            ''' Distance for slot orbit selection '''
            ...
            # bz x n_s x M x 3 ---> transformed part shape for each slot;
            # ori_pts: bz x 3 x N --> bz x N x 3 --> bz x 1 x 1 x N x 3
            selected_transformed_points = batched_index_select(values=transformed_pts, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(2)
            # print(f"selected_transformed_points, {selected_transformed_points.size()}")
            # dist_selected_transformed_pts_ori_pts: bz x n_s x M x N; transformed points to original points
            dist_selected_transformed_pts_ori_pts = torch.sum((selected_transformed_points.unsqueeze(3) - safe_transpose(ori_pts, -1, -2).unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
            dist_selected_transformed_pts_ori_pts, _ = torch.min(dist_selected_transformed_pts_ori_pts, dim=-2)
            # pts_slot_hard_labels: bz x N
            dist_selected_transformed_pts_ori_pts, pts_slot_hard_labels = torch.min(dist_selected_transformed_pts_ori_pts, dim=1)
            # print(f"dist_selected_trans_pts_to_ori_pts: {dist_selected_transformed_pts_ori_pts.size()}, pts_slot_hard_labels: {pts_slot_hard_labels.size()}")
            ''' Hard one hot labels... ''' #
            hard_one_hot_labels = torch.eye(self.num_slots).cuda()[pts_slot_hard_labels.long()] # hard_one_hot_labels: bz x N x n_s
            ...
            orbit_slot_dist_ori_recon_all_pts = minn_dist_recon_to_ori
            ...
            if self.recon_prior == 6 or self.recon_prior == 7 or self.recon_prior == 8 or self.recon_prior == 9:
                ...
                central_transformed_canon_points = slot_canon_pts + central_points.unsqueeze(
                    -2)  # central point transformed part shape reconstruction
                # print(
                ...
                # pair_dist_central_transformed_canon_points = torch.sum((central_transformed_canon_points[:, 0, ...].unsqueeze(-2) - central_transformed_canon_points[:, 1, ...].unsqueeze(-3)) ** 2, dim=-1)
                # minn_dist_central_transformed_a, _ = torch.min(pair_dist_central_transformed_canon_points, dim=-1)
                # minn_dist_central_transformed_b, _ = torch.min(pair_dist_central_transformed_canon_points, dim=-2)
                # # minn_dist_central_transformed_a: bz
                # minn_dist_central_transformed_a, _ = torch.min(minn_dist_central_transformed_a, dim=-1)
                # minn_dist_central_transformed_b, _ = torch.min(minn_dist_central_transformed_b, dim=-1)
                # minn_other_slots_base_canon_dist = minn_other_slots_base_canon_dist + (minn_dist_central_transformed_a + minn_dist_central_transformed_b).mean()
                ...
                # minn_other_slots_base_canon_dist = minn_other_slots_base_canon_dist + distance_transformed_pvp_pts
                #
            ...
            expanded_recon_slot_pts = expanded_recon_slot_pts.contiguous().view(bz * self.num_slots * self.recon_part_M,
                                                                                3)[
            ...
            attn_ori = safe_transpose(hard_one_hot_labels, -1, -2).float()
            if cur_iter == 0:
...
def build_model(opt,
    ...
    params['general'] = {
        ...
        'feat_partition': opt.equi_settings.feat_partition,
        # 'opt': opt
