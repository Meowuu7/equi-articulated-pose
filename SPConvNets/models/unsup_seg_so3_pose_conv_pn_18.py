...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.attn_in_dim = (self.encoded_feat_dim + self.kanchor) if self.orbit_attn == 1 else (
            self.kanchor) if self.orbit_attn == 2 else (self.encoded_feat_dim + 3) if self.orbit_attn == 3 else (
            self.encoded_feat_dim)
        self.slot_attention = SlotAttention(num_slots=params['outblock']['k'],
                                            dim=(self.encoded_feat_dim + self.kanchor) if self.orbit_attn == 1 else (
                                                self.kanchor) if self.orbit_attn == 2 else (
                                                        self.encoded_feat_dim + 3) if self.orbit_attn == 3 else (
                                                self.encoded_feat_dim), hidden_dim=self.encoded_feat_dim,
        ...
        for i_s in range(self.num_slots):
            ...
            self.deformation_flow_predict_per_part_net.append(
                nn.Sequential(
                    nn.Conv2d(
                        in_channels=self.encoded_feat_dim + 3 if self.recon_prior != 4 else self.encoded_feat_dim + self.atlas_prior_in_dim,
                        out_channels=self.encoded_feat_dim // 2, kernel_size=(1, 1),
                        stride=(1, 1), bias=True),
                    nn.BatchNorm2d(num_features=self.encoded_feat_dim // 2),
                    # nn.ReLU(),
                    nn.LeakyReLU(inplace=True),
        ...
        if self.recon_prior in [3, 4]:
            ...
            for i_s in range(self.num_slots):
                self.category_part_reconstruction_net_v2.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels=self.category_part_recon_v2_in_dim,
                                  out_channels=self.encoded_feat_dim // 2, kernel_size=(1, 1),
                                  stride=(1, 1), bias=True),
    ...
    def select_anchors_via_previous_consistency(self, dot_anchor_part_rots_anchors):
        ...
        print(cur_queue.size(), self.queue_len, self.queue_tot_len)
    ...
    def select_slot_orbits(self, slot_recon_loss, slot_pred_rots):
        ...
        if self.queue_len < 2:
            ...
            return dist_chamfer_recon_slot_ori, selected_slot_oribt, dist_chamfer_recon_slot_ori.unsqueeze(1), selected_slot_oribt.unsqueeze(1)
        ...
        topk_res_selected_anchors = []

        cur_k = self.annealing_k

        for i_bz in range(bz):
            tot_orbit_cmb = []
            tot_minn_loss = 1e8
            anchor_res_to_recon_loss = {}
            for i_a in range(self.kanchor):
                ...
                anchor_res_to_recon_loss[tuple([i_a] + minn_orbit_cmb)] = minn_recon_other_parts + cur_bz_anchor_part_recon_loss
            res_selected_anchors.append(tot_orbit_cmb)
            anchor_res_to_recon_loss_ordered_pairs = sorted(anchor_res_to_recon_loss.items(), key=lambda ii: ii[1], reverse=True)
            topk_anchor_res_to_recon_loss = anchor_res_to_recon_loss_ordered_pairs[: cur_k]
            topk_anchor_res_to_recon_loss = [list(iii[0]) for iii in topk_anchor_res_to_recon_loss] # convert tuple to list for further
            topk_res_selected_anchors.append(topk_anchor_res_to_recon_loss)

        #  topk_res_selected_anchors: bz x cur_k x n_slot
        topk_res_selected_anchors = torch.tensor(topk_res_selected_anchors, dtype=torch.long).cuda()

        # dot_rotations

        topk_res_selected_losses = []
        for i_k in range(cur_k):
            cur_k_selected_anchors = topk_res_selected_anchors[:, i_k, :]
            # cur_k_selected_loss: bz x n_s
            cur_k_selected_loss = batched_index_select(values=slot_recon_loss, indices=cur_k_selected_anchors.unsqueeze(-1), dim=2).squeeze(2)
            # cur_k_selected_loss: bz x 1 x n_s
            topk_res_selected_losses.append(cur_k_selected_loss.unsqueeze(1))
        topk_res_selected_losses = torch.cat(topk_res_selected_losses, dim=1)
        ...
        return res_selected_loss, res_selected_anchors, topk_res_selected_losses, topk_res_selected_anchors
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if self.inv_attn == 0:
            ...
            invariant_feats = batched_index_select(values=x.contiguous().permute(0, 2, 3, 1),
                                                   indices=selected_pts_orbit.unsqueeze(-1), dim=2)
        ...
        if self.orbit_attn > 0:
            ...
            if self.orbit_attn == 1:
                inv_orbit_feats = torch.cat(
                    [invariant_feats.contiguous().permute(0, 2, 1).contiguous(), orbit_attn_weights], dim=-1)
        ...
        if self.gt_oracle_seg == 1:  # oracle seg
            ...
            attn_ori = batched_index_select(values=attn_pred_gt_rel_pts, indices=attn_gt_selected_idx.unsqueeze(-1),
                                            dim=2)
        ...
        expaned_hard_one_hot_label = hard_one_hot_labels.unsqueeze(1).unsqueeze(-1).contiguous().repeat(1,
                                                                                                        transformed_variant_feats.size(
                                                                                                            1), 1, 1,
                                                                                                        self.kanchor)
        ...
        if self.feat_pooling == "mean":
            if self.soft_attn == 0:
                variant_feats_slot = torch.sum(
                    transformed_variant_feats.unsqueeze(2) * hard_one_hot_labels_slot.unsqueeze(1).unsqueeze(-1), dim=3)
            ...
            variant_feats_slot = torch.max(transformed_variant_feats, dim=2)[0].unsqueeze(2).repeat(1, 1,
                                                                                                    self.num_slots,
                                                                                                    1)
        else:
        ...
        expaned_hard_one_hot_label = hard_one_hot_labels.unsqueeze(1).contiguous().repeat(1,
                                                                                          transformed_variant_feats.size(
                                                                                              1), 1, 1)
        if self.feat_pooling == "mean":
            if self.soft_attn == 0:
                invariant_feats_slot = torch.sum(
                    invariant_feats.unsqueeze(-1).unsqueeze(2) * hard_one_hot_labels_slot.unsqueeze(1).unsqueeze(-1),
                    dim=3)

            elif self.soft_attn == 3:
                invariant_feats_slot = torch.mean(invariant_feats, dim=-1).unsqueeze(-1).repeat(1, 1,
                                                                                                self.num_slots).unsqueeze(
                    -1)
            elif self.soft_attn == 1:
            ...
            invariant_feats_slot = invariant_feats.unsqueeze(-1).unsqueeze(2) * hard_one_hot_labels.unsqueeze(
                1).unsqueeze(-1)
            invariant_feats_slot[expaned_hard_one_hot_label < 0.5] = -9999999.0
            invariant_feats_slot, _ = torch.max(invariant_feats_slot, dim=3)
            invariant_feats_slot = torch.max(invariant_feats, dim=-1)[0].unsqueeze(-1).repeat(1, 1,
                                                                                              self.num_slots).unsqueeze(
                -1)
        else:
            raise ValueError(f"Unrecognized feature pooling matrix: {self.feat_pooling}.")
        ...
        if self.recon_prior == 0:
            ...
            # recon_slot_points: bz x n_s x M x 3
            recon_slot_points, flow_reg_losses = self.apply_part_deformation_flow_net_instance(category_pts,
        ...
        if self.soft_attn == 0:
            ...
            dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * hard_one_hot_labels.unsqueeze(-1),
                                                       dim=2) / torch.clamp(
                torch.sum(hard_one_hot_labels.unsqueeze(-1), dim=2), min=1e-9)
            dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * hard_one_hot_labels.unsqueeze(-1),
                                                       dim=2)
        else:
            ...
            dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * hard_one_hot_labels.unsqueeze(-1),
                ...
                torch.sum(hard_one_hot_labels.unsqueeze(-1), dim=2), min=1e-9)
            dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * hard_one_hot_labels.unsqueeze(-1),
                                                       dim=2)  # / torch.clamp(torch.sum(hard_one_hot_labels.unsqueeze(-1), dim=2), min=1e-9)
        ...
        dist_chamfer_recon_slot_ori, selected_slot_oribt, topk_res_selected_losses, topk_res_selected_anchors = self.select_slot_orbits(dist_chamfer_recon_slot_ori, pred_R)

        # if self.queue_len == 0:

        avg_dist_chamfer_recon_slot_ori_losses = []
        for i_k in range(topk_res_selected_anchors.size(1)):
            # cur_res_selected_anchors: bz x n_s
            cur_res_selected_anchors = topk_res_selected_anchors[:, i_k, :]

            cur_k_avg_dist_chamfer_recon_slot_ori = torch.sum(slot_weights * topk_res_selected_losses[:, i_k, :], dim=-1)
            avg_dist_chamfer_recon_slot_ori_losses.append(cur_k_avg_dist_chamfer_recon_slot_ori)

            # print(dist_chamfer_recon_ori_to_slot.size(), selected_slot_oribt.size())

        selected_dist_chamfer_recon_ori_to_slot = batched_index_select(values=dist_chamfer_recon_ori_to_slot,
                                                                       indices=selected_slot_oribt.unsqueeze(-1), dim=2)
        ...
        for i_k in range(topk_res_selected_anchors.size(1)):
            cur_res_selected_anchors = topk_res_selected_anchors[:, i_k, :]
            cur_res_selected_recon_slot_points = batched_index_select(values=transformed_slot_pts,
                                                          indices=cur_res_selected_anchors.unsqueeze(-1), dim=2).squeeze(2)

            # bz x (n_s * M) x 3
            cur_expanded_recon_slot_points = (
                    cur_res_selected_recon_slot_points * hard_slot_indicator.unsqueeze(-1).unsqueeze(-1)).contiguous().view(bz, self.num_slots * self.recon_part_M, 3)

            ''' Sample points from ori_recon_slot_points '''
            cur_fps_idx = farthest_point_sampling(cur_expanded_recon_slot_points, npoints)
            # sampled_recon_pts: bz x N x 3
            cur_sampled_recon_pts = cur_expanded_recon_slot_points.contiguous().view(bz * (self.recon_part_M) * (self.num_slots),
                -1)[cur_fps_idx, :].contiguous().view(bz, npoints, -1)
            ''' Recon to Ori dist --- v1 '''

            if i_k == 0:
                selected_recon_slot_points = cur_res_selected_recon_slot_points
                sampled_recon_pts = cur_sampled_recon_pts

            ''' Sampled-Ori Chamfer distance hand-crafted calculation version '''
            # bz x npoints x npoints
            cur_glb_dist_recon_ww_ori = torch.sum((cur_sampled_recon_pts.unsqueeze(2) - ori_pts.unsqueeze(1)) ** 2, dim=-1)
            # glb_dist_recon_ww_ori = torch.sqrt(glb_dist_recon_ww_ori)
            cur_glb_dist_recon_to_ori, _ = torch.min(cur_glb_dist_recon_ww_ori, dim=-1)
            cur_glb_dist_ori_to_recon, _ = torch.min(cur_glb_dist_recon_ww_ori, dim=-2)

            # bz
            # glb_dist_recon_to_ori = glb_dist_recon_to_ori.mean(dim=-1) # .mean()
            cur_glb_dist_recon_to_ori = cur_glb_dist_recon_to_ori.sum(dim=-1)  # .mean()
            # bz
            # glb_dist_ori_to_recon = glb_dist_ori_to_recon.mean(dim=-1) # .mean()
            cur_glb_dist_ori_to_recon = cur_glb_dist_ori_to_recon.sum(dim=-1)  # .mean()

            cur_tot_recon_loss = self.glb_recon_factor * (cur_glb_dist_ori_to_recon + cur_glb_dist_recon_to_ori) + self.slot_recon_factor * avg_dist_chamfer_recon_slot_ori_losses[i_k]

            tot_recon_losses  = tot_recon_losses + cur_tot_recon_loss.mean()

        tot_recon_loss = tot_recon_losses / topk_res_selected_anchors.size(1)
