...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        for i_s in range(self.num_slots):
            ...
            cur_r_representation = self.r_representation if i_s > 0 else 'quat' # use quat representation for the model of this version...
        ...
        if self.stage == 0:
            ...
            if x_list is None:
                ...
                if not self.use_art_mode:
                    ...
                    ''' Process points via changing convolution process '''
                    x_w_art_mode = []
                    for cur_x in x_list:
            ...
            ''' Get hard labels and seg_to_idxes... '''
            # hard_labels: bz x n2; hard_labels...
            if self.gt_oracle_seg:
                # print(f"using gt_orcale_seg..., rlabel: {rlabel.size()}")
                # attn_ori = rlabel
                attn_ori = safe_transpose(rlabel, -1, -2)
            hard_labels = torch.argmax(attn_ori, dim=1)
            # hard_labels = torch.argmax(rlabel, dim=1)
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    # if self.pred_pv_equiv: # pv points and central points from equiv-features
            ...
            for i_s in range(len(slot_R)):  # get slots' rotations
                ...
                for i_bz in range(bz):  # seg label for the base part?
                    cur_bz_cur_minn_seg_label = tot_minn_seg_labels[i_bz] # keep non-motion...
                    ##### Slot 1's predicted axis #####
                    defined_axis = slot_axis[i_bz, 1]  ### the second slot's aixs... # use the second slots' predicted axis?
                    ...
                    if i_s == cur_bz_cur_minn_seg_label:
                        ...
                        cur_slot_R_mtx = slot_R[i_s][i_bz].unsqueeze(
                            0)
                        if self.shape_type == 'drawer_zz':
                            cur_slot_R_mtx = cur_slot_R_mtx * 0.0
                        ...
                        # print(f"cur_slot_R_mtx: {cur_slot_R_mtx.size()}")
                        # cur_slot_R_mtx: bz x n_s x
                        cur_slot_R_mtx = compute_rotation_matrix_from_quaternion(
                            safe_transpose(cur_slot_R_mtx, -1, -2).view(1 * 1 * cur_kanchor, -1).contiguous()
                        ).contiguous().view(1, 1, cur_kanchor, 3, 3).contiguous()

                    tot_cur_slot_mtx_R.append(cur_slot_R_mtx) # put slot rotation matrix to the total slot rotation matrix list...
            ...
            #### Shape type --> drawer #####
            if self.shape_type == 'drawer_zz':  # drawer; drawer...
            ...
            # select a specific mode for transformation
            ...
            # print(f"orbit_slot_dist_ori_recon_all_pts: {orbit_slot_dist_ori_recon_all_pts.size()}, slot_orbits: {slot_orbits.size()}")
            ...
            # print(f"k: {k}, cur_kanchor: {cur_kanchor}, minn_dist_ori_to_recon: {minn_dist_ori_to_recon.size()}, slot_oribts: {slot_orbits}")
            ...
            slot_R_raw = safe_transpose(slot_R_raw, -1, -2)
            # print(f"check slot_R: slot_R: {slot_R.size()}, slot_T: {slot_T.size()}, slot_orbits: {slot_orbits.size()}, slot_T_joint: {slot_T_joint.size()}, slot_R_raw: {slot_R_raw.size()}")
