...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        ''' Construct inv-feat output block for slot reconstruction... '''
        #### glb_outblock_for_slot ####
        self.glb_outblock_for_slot = Mso3.InvOutBlockOursWithMask(params['outblock'], norm=1, pooling_method='attention',
                                                         use_pointnet=True)
        ''' Construct inv-feat output block for slot reconstruction... '''
        ...
        for i_s in range(self.num_slots):
            if self.recon_prior == 4:
                ...
                self.slot_shp_recon_net.append(
                    DecoderConstantCommon([256, 256], params['outblock']['mlp'][-1], self.recon_part_M, None, use_sigmoid=self.use_sigmoid)
                )
                ...
                # print("self.use_sigmoide: ", self.use_sigmoid)
                # self.use_sigmoid = False
                self.slot_shp_recon_net.append(
                    DecoderFC([256, 256], params['outblock']['mlp'][-1], self.recon_part_M, None, use_sigmoid=self.use_sigmoid)
                )
        ...
        for i_s in range(self.num_slots):
            ...
            cur_r_representation = self.r_representation
        ...
        self.extra_trans_outblk_RT = SO3OutBlockRTWithAxisWithMask(params['outblock'], norm=1, pooling_method='max',
                                          global_scalar=True,
                                          # global scalar?
                                          use_anchors=False,
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if self.stage == 0:
            ...
            slot_inv_feats, slot_orbit_confidence = self.glb_outblock_for_slot(x, mask=None)
            ...
            cur_extra_xyz = sptk.SphericalPointCloud(x.xyz, x.feats, x.anchors)
            extra_RT = self.extra_trans_outblk_RT(cur_extra_xyz, mask=None,
                                                                   anchors=self.anchors.unsqueeze(0).repeat(
                                                                       bz, 1, 1, 1).contiguous(),
                                                                   proj_axis=None, use_offset=False)
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz): # xyz features
                    # print(f"check x, x.xyz: {x.xyz.size()}, x.feats: {x.feats.size()}, x.anchors: {x.anchors.size()}")
                    # x.xyz: bz x 3 x N
                    # cur_bz_cur_slot_xyz: 1 x N x 3
                    # cur_bz_cur_slot_feats: 1 x N x dim
                    ''' Coordiantes and features of the whole input shape for part shape reconstruction and pose prediction '''
                    ...
                    ''' If we use invariant features for slots' angle decoding '''
                    # Get expanded slot's inv feats
                    # expanded_cur_slot_inv_feats = cur_bz_cur_slot_inv_feats.contiguous().unsqueeze(-1).unsqueeze(-1).repeat(1, 1, x.xyz.size(-1), x.feats.size(-1))
                    # cur_bz_cur_slot_x = sptk.SphericalPointCloud(safe_transpose(cur_bz_cur_slot_xyz, -1, -2),
                    #                                              expanded_cur_slot_inv_feats, x.anchors)
                    ''' If we use invariant features for slots' angle decoding '''
                    ...
                    if self.shape_type == 'drawer':
                        ...
                        cur_bz_slot_output_RT = self.slot_trans_outblk_RT[i_s](cur_bz_cur_slot_x, mask=None,
                                                                               anchors=self.anchors.unsqueeze(0).repeat(
                                                                                   bz, 1, 1, 1).contiguous(), proj_axis=defined_proj_axis, use_offset=use_offset)
                    else:
                ...
                all_slots_inv_feats.append(cur_slot_inv_feats.unsqueeze(1))
                ...
                if i_s >= self.num_slots - 3:
                    ...
                    maxx_ori_pts_x_coord = torch.tensor([0.20], dtype=torch.float32).cuda().unsqueeze(0).repeat(bz, 1)
                    minn_ori_pts_x_coord = torch.tensor([-0.20], dtype=torch.float32).cuda().unsqueeze(0).repeat(bz, 1)
                    ...
                    maxx_ori_coord_y = torch.tensor([0.36], dtype=torch.float32).cuda().unsqueeze(0).repeat(bz, 1)
                    minn_ori_coord_y = torch.tensor([-0.36], dtype=torch.float32).cuda().unsqueeze(0).repeat(bz, 1)

                    minn_pts_coord[:, 0, :] = minn_ori_pts_x_coord + 0.5

                    if i_s == self.num_slots - 3:
                        minn_pts_coord[:, 1, :] = (1.0 - (maxx_ori_coord_y - minn_ori_coord_y)) / 2.0 + 0.05

                    elif i_s == self.num_slots - 2:
                        # minn_pts_coord[:, 1, :] = 0.3
                        minn_pts_coord[:, 1, :] = (maxx_ori_coord_y - minn_ori_coord_y) / 3. + (
                                    1.0 - (maxx_ori_coord_y - minn_ori_coord_y)) / 2.0
                    elif i_s == self.num_slots - 1:
                        # minn_pts_coord[:, 1, :] = 0.6
                        minn_pts_coord[:, 1, :] = (maxx_ori_coord_y - minn_ori_coord_y) / 3. * 2. + (
                                    1.0 - (maxx_ori_coord_y - minn_ori_coord_y)) / 2.0

                    cur_slot_canon_pts = zeross_pts_coord * scale_factor + minn_pts_coord
                    print("max and min", torch.max(cur_slot_canon_pts, dim=-1)[0],
                          torch.min(cur_slot_canon_pts, dim=-1)[0])
            ...
            # slot_canon_pts: bz x n_s x M x 3 # slot
            # slot_R: bz x n_s x 4 x na # slot rotation matrix
            # slot_T: bz x n_s x 3 x na # slot transaltion matrix
            slot_canon_pts = torch.cat(slot_canon_pts, dim=1)
            slot_canon_pts = safe_transpose(slot_canon_pts, -1, -2)

            all_slots_inv_feats = torch.cat(all_slots_inv_feats, dim=1)

            defined_axises = torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32).cuda().unsqueeze(0).unsqueeze(0).repeat(bz, self.kanchor, 1).contiguous()
            ...
            mtx_slot_R = []
            for i_s in range(len(slot_R)):
                if i_s >= 0:
                    # slot-R-matrix
                    # current slot-R rotation matrix...

                    cur_slot_R_mtx = compute_rotation_matrix_from_quaternion(
                    safe_transpose(slot_R[3], -1, -2).view(bz * cur_kanchor * 1, -1)).contiguous().view(bz, 1, cur_kanchor, 3, 3)

                    # cur_slot_R_mtx = compute_rotation_matrix_from_quaternion(
                    #     safe_transpose(extra_RT['R'], -1, -2).view(bz * cur_kanchor * 1, -1)).contiguous().view(bz, 1,
                    #                                                                                         cur_kanchor,
                    #                                                                                         3, 3)
                    ...
                    # use the defined axis and predicted angles
            ...
            # mtx_extra_R = compute_rotation_matrix_from_quaternion(
            #         safe_transpose(slot_R[3], -1, -2).view(bz * cur_kanchor * 1, -1)).contiguous().view(bz, 1, cur_kanchor, 3, 3)
            ...
            slot_R_ori = slot_R.clone().detach()
            ...
            ##### Shape type --> drawer #####
            ''' Set the translation to zero.. orz... '''
            # if self.shape_type == 'drawer':
            ...
            # transformed_pts: bz x ns x na x M x 1 x 3 .... -> bz x ns x na x M x N
            ...
            orbit_slot_dist_ori_recon = minn_dist_recon_to_ori_all_pts
            ...
            # bz x n_s x M x N
            selected_dist_recon_ori = batched_index_select(values=dist_recon_ori, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(2)
            minn_ori_to_slot_dist, _ = torch.min(selected_dist_recon_ori, dim=-2)
            minn_ori_to_slot_dist, pts_labels = torch.min(minn_ori_to_slot_dist, dim=1)

            hard_one_hot_labels = torch.eye(self.num_slots, dtype=torch.float32).cuda()[pts_labels.long()]
            ...
            dist_recon_ori[expanded_hard_one_hot_labels < 0.5] = 99999.0
            minn_dist_recon_to_ori, _ = torch.min(dist_recon_ori, dim=-1) # dist_recon_o
            # minn_dist_recon_to_ori: bz x n_s x na x M --> bz x n_s x na
            minn_dist_recon_to_ori = minn_dist_recon_to_ori.mean(-1)

            # minn_dist_ori_to_recon: bz x n_s x na x N
            minn_dist_ori_to_recon, _ = torch.min(dist_recon_ori, dim=-2) # ori to reconstruction

            soft_weights = safe_transpose(hard_one_hot_labels, -1, -2)
            ...
            ''' Bi-directional reconstruction loss --- ori shape to reconstruction + recon to ori shape '''
            orbit_slot_dist_ori_recon_all_pts = minn_dist_ori_to_recon + minn_dist_recon_to_ori
            #### slot distance recon all pts ####
            ''' Single-directional reconstruction loss --- recon to ori shape '''
            # orbit_slot_dist_ori_recon_all_pts = minn_dist_recon_to_ori # whether to use
            ''' Single-directional reconstruction loss --- recon to ori shape all points '''
            # orbit_slot_dist_ori_recon_all_pts = minn_dist_recon_to_ori_all_pts # whether to use
            # orbit_slot_dist_ori_recon_all_pts = minn_dist_recon_to_ori_all_pts + minn_dist_ori_to_recon
            ...
            print(f"slot_orbits: {slot_orbits}")
            ...
            elif self.recon_prior == 6: # slot pivot points --- for pivot points and others
            ...
            # print("zzz", selected_anchors.size(), defined_axises.size())
            real_defined_axises = torch.matmul(selected_anchors[:,0], defined_axises.unsqueeze(-1)).squeeze(-1)
            ...
            ##### PPInvariant features #####
            out_feats['ppinv_out'] = ppinv_out.detach().cpu().numpy() # shape related features
            ##### SlotInvariant features #####
            out_feats['all_slots_inv_feats'] = all_slots_inv_feats.detach().cpu().numpy() # shape related features
            np.save(self.log_fn + f"with_feats_n_stage_{self.stage}_iter_{cur_iter}.npy", out_feats)
