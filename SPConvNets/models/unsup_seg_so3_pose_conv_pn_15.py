...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.buffer_slot_rel_Rs = nn.Parameter(
            torch.zeros((self.buf_max_n, self.num_slots, 3, 3), requires_grad=False),
            requires_grad=False)
        self.buf_st = 0
        self.buf_n = 0
    ...
    def select_slot_orbits_queue(self, slot_recon_loss, slot_pred_rots):
        # slot_recon_loss: bz x n_s x na
        # slot_pred_rots: bz x n_s x na x 3 x 3
        # selected_orbit: bz
        _, selected_orbit = torch.min(slot_recon_loss[:, 0, ...], dim=1)
        selected_pred_rot = batched_index_select(values=slot_pred_rots[:, 0, ...], indices=selected_orbit.unsqueeze(1), dim=1)
        # selected_pred_rot: bz x 3 x 3
        selected_pred_rot = selected_pred_rot.squeeze(1)
        # mult_selected_others_rot: bz x n_s x na x 3 x 3
        mult_selected_others_rot = torch.matmul(selected_pred_rot.unsqueeze(1).unsqueeze(1), safe_transpose(slot_pred_rots, -1, -2))
        # buffer_slot_rel_Rs: buf_n x n_s x 3 x 3
        # then we should get relative rotations from selected rotations and buffered rotations
        # rel_curr_buf_rot: bz x buf_n x n_s x na x 3 x 3
        rel_curr_buf_rot = torch.matmul(mult_selected_others_rot.unsqueeze(1), safe_transpose(self.buffer_slot_rel_Rs.data.unsqueeze(0).unsqueeze(3), -1, -2))
        ax_x, ax_y, ax_z = rel_curr_buf_rot[..., 2, 1] - rel_curr_buf_rot[..., 1, 2], rel_curr_buf_rot[..., 0, 2] - rel_curr_buf_rot[..., 2, 0], rel_curr_buf_rot[..., 1, 0] - rel_curr_buf_rot[..., 0, 1]
        # axises: bz x buf_n x n_s x na x 3
        axises = torch.cat([ax_x.unsqueeze(-1), ax_y.unsqueeze(-1), ax_z.unsqueeze(-1)], dim=-1)
        # axises_dots: bz x buf_n x buf_n x n_s x na
        axises_dots = torch.sum(axises.unsqueeze(2) * axises.unsqueeze(1), dim=-1)
        # axises_dots: bz x n_s x na
        axises_dots = torch.mean(torch.mean(axises_dots, dim=2), dim=1)

        k = 4
        # topk_chamfer_dist: bz x n_s x 4; topk_dist_idxes: bz x n_s x 4
        topk_chamfer_dist, topk_dist_idxes = torch.topk(slot_recon_loss, k=k, dim=-1, largest=False)
        # topk_axises_dots: bz x n_s x 4
        topk_axises_dots = batched_index_select(values=axises_dots, indices=topk_dist_idxes, dim=2)
        # largest_dot_idxes: bz x n_s
        largest_dot_val, largest_dot_idxes = torch.max(topk_axises_dots, dim=-1)
        largest_dot_idxes = batched_index_select(values=topk_dist_idxes, indices=largest_dot_idxes.unsqueeze(-1), dim=2)
        # largest_dot_idxes: bz x n_s
        largest_dot_idxes = largest_dot_idxes.squeeze(-1)
        # largest_dot_idxes: bz x n_s
        largest_dot_idxes = torch.cat([selected_orbit.unsqueeze(-1), largest_dot_idxes], dim=-1)
        # selected_recon_loss: bz x n_s
        selected_recon_loss = batched_index_select(values=slot_recon_loss, indices=largest_dot_idxes.unsqueeze(-1), dim=2).squeeze(2)
        return selected_recon_loss, largest_dot_idxes
