...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        # self.slot_pair_mult_R_queue = torch.zeros((self.queue_tot_len, self.num_slots, 3, 3), dtype=torch.float,
        #                                           requires_grad=False).cuda()
        self.slot_pair_mult_R_queue = torch.zeros((self.queue_tot_len, self.num_slots ** 2, 3, 3), dtype=torch.float,
                                                  requires_grad=False).cuda()
        self.slot_pair_mult_R_queue = nn.Parameter(self.slot_pair_mult_R_queue, requires_grad=False)

        ''' Construct pointnet block for each slot '''
        self.slot_pointnet = nn.ModuleList()
        for i_s in range(self.num_slots):
            cur_pointnet = Mso3.ClsOutBlockR(params['outblock'], out_task=False)
            self.slot_pointnet.append(cur_pointnet)
    ...
    def select_slot_orbits(self, slot_recon_loss, slot_pred_rots):
        ...
        # dot_anchor_part_rots_anchors = torch.matmul(anchor_part_rots.unsqueeze(1).unsqueeze(3), slot_pred_rots.unsqueeze(2).transpose(-1, -2).contiguous())
    ...
    def get_axis_consistency_loss(self, dot_anchor_part_rots_anchors):
        # dot_anchor_part_rots_anchors: bz x n_s x 3 x 3
        ...
        cur_q_len = cur_queue.size(0)
        bz = dot_anchor_part_rots_anchors.size(0)
        # cur_queue = cur_queue.contiguous().view(cur_q_len, self.num_slots, self.num_slots, 3, 3).contiguous()

        in_q_n = cur_queue.size(1)
        # cur_queue: queue_len x n_s x 3 x 3
        # dot_rots_cur_queue: bz x queue_len x n_s x 3 x 3
        # dot_rots_cur_queue = torch.matmul(dot_anchor_part_rots_anchors.unsqueeze(1), cur_queue.view(1, self.queue_len, self.num_slots, 3, 3).contiguous())
        dot_rots_cur_queue = torch.matmul(dot_anchor_part_rots_anchors.unsqueeze(1), cur_queue.view(1, self.queue_len, in_q_n, 3, 3).contiguous())
        ...
        # dot_axis: bz x queue_len x n_s x 3
        ...
        # dot_axis_between_queue: bz x q_l x q_l x n_s
        ...
        ''' Previous: trivial mean '''
        # dot_axis_between_queue_aggr: bz x n_s
        # dot_axis_between_queue_aggr = dot_axis_between_queue.mean(1).mean(1)
        ''' Current: use the mask to filter self-self axis dot-product values '''
        queue_sqr_mask = 1. - torch.eye(cur_q_len, dtype=torch.float).cuda()
        dot_axis_between_queue_aggr = (dot_axis_between_queue * queue_sqr_mask.unsqueeze(0).unsqueeze(-1)).sum(1).sum(1) / queue_sqr_mask.sum()

        #
        slot_sqr_mask = 1. - torch.eye(self.num_slots, dtype=torch.float).cuda()
        # slot_sqr_mask: 1 x n_s x n_s
        slot_sqr_mask = slot_sqr_mask.unsqueeze(0)
        # hard_slot_indicators: bz x n_s
        # slot_sqr_mask: bz x n_s x n_s
        slot_sqr_mask = slot_sqr_mask * self.hard_slot_indicators.unsqueeze(-1)
        slot_sqr_mask = slot_sqr_mask * self.hard_slot_indicators.unsqueeze(-2)
        activated_slots = torch.sum(self.hard_slot_indicators, dim=-1)
        ''' Current: use the mask to filter self-self slot dot-product values '''
        dot_axis_between_queue_aggr = dot_axis_between_queue_aggr.contiguous().view(bz, self.num_slots, self.num_slots).contiguous()
        # dot_axis_between_queue_aggr = (dot_axis_between_queue_aggr * slot_sqr_mask.unsqueeze(0)).sum(1).sum(1) / self.num_slots
        dot_axis_between_queue_aggr = (dot_axis_between_queue_aggr * slot_sqr_mask.unsqueeze(0)).sum(1).sum(1) / activated_slots
        dot_axis_between_queue_aggr_loss = -1.0 * dot_axis_between_queue_aggr.mean()
        ''' Previous: trivial sum and mean '''
        # dot_axis_between_queue_aggr_loss = -1.0 * dot_axis_between_queue_aggr.sum(-1).mean()
        return dot_axis_between_queue_aggr_loss
    ...
    def apply_slot_pointnet(self, feats, slot_one_hot_labels):
        # feats: bz x dim x N x na
        # slot_one_hot_labels: bz x n_s x N
        slot_oribt_weights = []
        for i_s in range(self.num_slots):
            cur_slot_one_hot_labels = slot_one_hot_labels[:, i_s, :]
            # cur_slot_orbit_weights: bz x N
            _, cur_slot_orbit_weights = self.slot_pointnet[i_s](feats, mask=cur_slot_one_hot_labels)
            if len(cur_slot_orbit_weights.size()) == 1:
                cur_slot_orbit_weights = cur_slot_orbit_weights.unsqueeze(0)
            slot_oribt_weights.append(cur_slot_orbit_weights.unsqueeze(1))
        # slot_orbit_weights: bz x n_s x na
        slot_oribt_weights = torch.cat(slot_oribt_weights, dim=1)
        return slot_oribt_weights
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        hard_slot_indicators = (hard_slot_weights > 0.5).float()
        self.hard_slot_indicators = hard_slot_indicators
        ...
        if self.feat_pooling == "mean":
            ...
            variant_feats_slot = torch.max(transformed_variant_feats, dim=2)[0].unsqueeze(2).repeat(1, 1, self.num_slots, 1)
        else:
            raise ValueError(f"Unrecognized feature pooling matrix: {self.feat_pooling}.")

        # slot_orbit_weights: bz x n_s x na
        slot_orbit_weights = self.apply_slot_pointnet(feats=transformed_variant_feats, slot_one_hot_labels=hard_one_hot_labels)

        ''' Get invariant features for each slot '''
        ...
        ''' Use invariant for canonical points reconstruction '''
        ...
        ''' Select just via reconstruction loss '''
        # dist_chamfer_recon_slot_ori, selected_slot_oribt = torch.min(dist_chamfer_recon_slot_ori, dim=2)
        ''' Select via reconstruction loss and some constraints '''
        # dist_chamfer_recon_slot_ori, selected_slot_oribt = self.select_slot_orbits(dist_chamfer_recon_slot_ori, pred_R)
        ''' Select via orbit attention weights '''
        # slot_orbit_weights: bz x n_s x na
        # selected_slot_oribt: bz x n_s
        if len(slot_orbit_weights.size()) == 2:
            slot_orbit_weights = slot_orbit_weights.unsqueeze(0)
        selected_slot_oribt = torch.argmax(slot_orbit_weights, dim=-1, keepdim=False)

        # selected_pred_R: bz x n_s x 3 x 3
        # print(pred_R.size(), selected_slot_oribt.size())
        selected_pred_R = batched_index_select(values=pred_R, indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(2)
        # rel_pred_R: bz x n_s x 3 x 3
        # rel_pred_R = torch.matmul(selected_pred_R[:, 0, ...].unsqueeze(1).transpose(-1, -2).contiguous(), selected_pred_R)

        rel_pred_R = torch.matmul(selected_pred_R.unsqueeze(2).transpose(-1, -2).contiguous(), selected_pred_R.unsqueeze(1))
        rel_pred_R = rel_pred_R.contiguous().view(bz, self.num_slots ** 2, 3, 3).contiguous()

        if self.queue_len > 1:
            axis_consistency_loss = self.get_axis_consistency_loss(rel_pred_R)
        else:
            axis_consistency_loss = None
        dots_pred_axis_with_prior_loss = axis_consistency_loss
        self.update_slot_pair_mult_R_queue(rel_pred_R)

        ''' Previous axis-based loss '''
        # axis_x, axis_y, axis_z = rel_pred_R[..., 2, 1] - rel_pred_R[..., 1, 2], rel_pred_R[..., 0, 2] - rel_pred_R[..., 2, 0],  rel_pred_R[..., 1, 0] - rel_pred_R[..., 0, 1]
        # # rel_pred_axis: bz x n_s x 3
        # rel_pred_axis = torch.cat([axis_x.unsqueeze(-1), axis_y.unsqueeze(-1), axis_z.unsqueeze(-1)], dim=-1)
        # rel_pred_axis = rel_pred_axis / torch.clamp(torch.norm(rel_pred_axis, p=2, dim=-1, keepdim=True), min=1e-8)
        # # dots_pred_axis_with_prior: bz x n_s
        # dots_pred_axis_with_prior = torch.sum(rel_pred_axis * self.axis_prior_slot_pairs.unsqueeze(0), dim=-1)
        # dots_pred_axis_with_prior_loss = -1. * dots_pred_axis_with_prior[:, 1:].mean()
        #

        # print(f"dots_pred_axis_with_prior_loss: {dots_pred_axis_with_prior_loss.item()}")

        # self.update_slot_pair_axis_prior(rel_pred_axis)

        # add supervisions for orbit weights
        # we need sone slots for prediction
        # maximize slots' weights for the top 5 ranked chamfer loss
        # remember to multiply with slot indicators!
        # topk_chamfer_loss_orbit_idxes: bz x n_s x 5


        _, topk_chamfer_loss_orbit_idxes = torch.topk(dist_chamfer_recon_slot_ori, k=1, largest=False)
        slot_orbit_weights = torch.log_softmax(slot_orbit_weights, dim=-1)
        # topk_orbit_atten_weights: bz x n_s x 5
        topk_orbit_atten_weights = batched_index_select(values=slot_orbit_weights, indices=topk_chamfer_loss_orbit_idxes, dim=2)
        slot_atten_weights_loss = -1 * torch.mean((topk_orbit_atten_weights).mean(dim=-1) * hard_slot_indicator, dim=1).mean()
        print(f"slot_atten_weights_loss: {slot_atten_weights_loss.item()}")

        # dist_chamfer_recon_slot_ori: bz x n_s
        dist_chamfer_recon_slot_ori = batched_index_select(values=dist_chamfer_recon_slot_ori, indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(-1)
        ...
        tot_loss = tot_loss + slot_atten_weights_loss

        if dots_pred_axis_with_prior_loss is not None:
            print(f"dots_pred_axis_with_prior_loss: {dots_pred_axis_with_prior_loss.item()}")
            tot_loss = tot_loss + 10 * dots_pred_axis_with_prior_loss
