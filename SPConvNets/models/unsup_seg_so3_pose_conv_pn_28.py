...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
            ...
            if not self.pred_t:
                self.slot_trans_outblk_RT.append(
                    SO3OutBlockRWithMask(params['outblock'], norm=1, pooling_method='max', pred_t=self.pred_t,
                                         feat_mode_num=self.kanchor, representation=self.r_representation, num_in_channels=self.inv_out_dim)
                )
            else:
                self.slot_trans_outblk_RT.append(
                    SO3OutBlockRTWithMask(params['outblock'], norm=1, pooling_method='max',
                                          ...
                                          # global scalar?
                                          use_anchors=False,
                                          feat_mode_num=self.kanchor, num_heads=1, representation=self.r_representation, num_in_channels=self.inv_out_dim)
                )
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        #
        #     # transform normals for cuboid loss computing use #
        ...
        # glb_R: bz x na x 3 x 3; glb_T: bz x na x 3
        ...
        for i_s in range(self.num_slots):
            ...
            for i_bz in range(bz):
                ...
                # cur_bz_cur_slot_inv_feats: bz (1) x dim --> inv feature for this slot in the current batch
                # features for each point?; bz (1) x N x dim x na
                #
                cur_bz_cur_slot_plane_x = sptk.SphericalPointCloud(safe_transpose(cur_bz_cur_slot_xyz, -1, -2), cur_bz_cur_slot_inv_feats.unsqueeze(-1).unsqueeze(2).contiguous().repeat(1, 1, cur_bz_cur_slot_xyz.size(1), 1), x.anchors[29].unsqueeze(0))
                cur_bz_slot_output_RT = self.slot_trans_outblk_RT[i_s](cur_bz_cur_slot_plane_x, mask=None, anchors=self.anchors[29].unsqueeze(0).unsqueeze(0).repeat(bz, 1, 1, 1).contiguous())
            ...
            #### Canonical points reconstruction ####
        ...
        if self.r_representation == 'quat':
            ...
            definded_axis = torch.tensor([0.0, 1.0, 0.0], dtype=torch.float32).cuda().unsqueeze(0)
            slot_R = torch.sigmoid(slot_R) * math.pi
            # Get slot_R from .... and defined axis!!!
            slot_R = compute_rotation_matrix_from_angle(
                self.anchors[29].unsqueeze(0),
                safe_transpose(slot_R, -1, -2).view(bz * self.num_slots, 1, -1), defined_axis=definded_axis
            ).contiguous().view(bz, self.num_slots, 1, 3, 3).contiguous()
        ...
        # Get transformed pts in the plane
        # transformed_pts: bz x n_s x na (=1) x 3 x M
        transformed_pts = safe_transpose(torch.matmul(slot_R, safe_transpose(slot_canon_pts.unsqueeze(2), -1, -2)), -1,
                                         -2) + slot_T.unsqueeze(-2)

        # glb_transformed_pts: bz x n_s x na x 3 x M
        transformed_pts = torch.matmul(glb_R.unsqueeze(1), safe_transpose(transformed_pts, -1, -2)) + glb_T.unsqueeze(1).unsqueeze(-1)
        transformed_pts = safe_transpose(transformed_pts, -1, -2)

        slot_R = torch.matmul(glb_R.unsqueeze(1), slot_R)
        slot_T = torch.matmul(glb_R.unsqueeze(1), slot_T.unsqueeze(-1)).squeeze(-1)
        ...
        expanded_hard_one_hot_labels = safe_transpose(hard_one_hot_labels, -1, -2).unsqueeze(2).unsqueeze(2).repeat(1, 1, cur_kanchor, self.recon_part_M, 1)
        ...
        orbit_slot_dist_ori_recon = torch.sum(hard_slot_indicators.unsqueeze(-1) * orbit_slot_dist_ori_recon, dim=1)


        # slot_dist_ori_recon: bz; slot_orbits: bz
        slot_dist_ori_recon, slot_orbits = torch.min(orbit_slot_dist_ori_recon, dim=-1)
        slot_dist_ori_recon = slot_dist_ori_recon.unsqueeze(-1).contiguous().repeat(1, self.num_slots)
        slot_orbits = slot_orbits.unsqueeze(-1).contiguous().repeat(1, self.num_slots)
