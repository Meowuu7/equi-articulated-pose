...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        # remember predicted Rs for each slot in the last 8 iterations
        self.buffer_slot_rel_Rs = nn.Parameter(torch.zeros((8, (self.num_slots * (self.num_slots - 1)) // 2 , 3, 3), requires_grad=False), requires_grad=False)
        self.buf_st = 0
        self.buf_n = 0
        self.buf_max_n = 8
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        if R_slot_cons_loss is not None:
            print(f"R_slot_cons_loss: {R_slot_cons_loss}")
            tot_loss += R_slot_cons_loss
