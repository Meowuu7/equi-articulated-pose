...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        slot_recon_nei_dist = self.get_slot_pts_dist_neis()
        # cur_slot_recon_ori_pts_dist: bz x n_a x n_s x M x (n_s + 1)
        cur_slot_recon_ori_pts_dist = self.calculate_ctx_dist(safe_transpose(transformed_slot_pts, 1, 2), ori_pts,
                                                              pred_labels=point_label, r=0.2, k=32)
        if slot_recon_nei_dist is not None:
            # curr_dist_scores: bz x n_a x n_s x M
            curr_dist_scores = self.calculate_matching_prob(slot_recon_nei_dist, cur_slot_recon_ori_pts_dist)
            # sum_dist_scores: bz x n_a x n_s
            sum_dist_scores = torch.sum(curr_dist_scores, dim=-1)
            sum_dist_scores = safe_transpose(sum_dist_scores, 1, 2)
        else:
            sum_dist_scores = None
        ...
        if sum_dist_scores is not None:
            # bz x ns x n_a
            print(f"sum_dist_scores: {sum_dist_scores.mean().item()}, chamfer_dist: {dist_chamfer_recon_slot_ori.mean().item()}") # print out mean of those scores to get some sense

            recon_slot_metric = dist_chamfer_recon_slot_ori + (-0.2) * sum_dist_scores
            # recon_slot_metric = dist_chamfer_recon_slot_ori +  sum_dist_scores
        else:
            recon_slot_metric = dist_chamfer_recon_slot_ori

        recon_slot_metric, selected_slot_oribt = torch.min(recon_slot_metric, dim=2)
        ...
        # cur_slot_recon_ori_pts_dist: bz x n_s x M x (n_s + 1)
        cur_slot_recon_ori_pts_dist = batched_index_select(values=safe_transpose(cur_slot_recon_ori_pts_dist, 1, 2), indices=selected_slot_oribt.unsqueeze(-1), dim=2).squeeze(2)

        ''' Update slot points' neighbours distribution '''
        self.update_slot_pts_dist_neis(cur_slot_recon_ori_pts_dist)
