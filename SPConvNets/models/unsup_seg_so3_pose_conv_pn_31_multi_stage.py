...
from SPConvNets.models.model_utils import SO3OutBlockRTWithAxisWithMask, SO3OutBlockRTWithMask
...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.use_2d = params['general']['use_2d']
        ...
        self.tot_anchors = torch.matmul(self.anchors.unsqueeze(1), self.plane_anchors.unsqueeze(0))
        self.tot_anchors = self.tot_anchors.contiguous().view(self.anchors.size(0) * self.plane_anchors.size(0), 3, 3).contiguous()
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if self.stage == 0:
            ...
            # Get 2D anhcors for input
            x = M.preprocess_input(x, cur_kanchor if not self.use_2d else cur_kanchor * planeconv_kanchor, pose, False)
            ...
            if self.inv_pooling_method == 'attention':
                ...
                # Get invariant features from all features
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    # Here, number of anchors in x.feats is kanchor * 4 (n_plane_anchor)
                    ...
                    if self.shape_type == 'drawer':
                        ...
                        cur_bz_slot_output_RT = self.slot_trans_outblk_RT[i_s](cur_bz_cur_slot_x, mask=None,
                                                                               anchors=self.tot_anchors.unsqueeze(0).repeat(
                                                                                   bz, 1, 1, 1).contiguous(), proj_axis=defined_proj_axis)
                    else: # cur_bz_slot_output
                        #
                        cur_bz_slot_output_RT = self.slot_trans_outblk_RT[i_s](cur_bz_cur_slot_x, mask=None, anchors=self.tot_anchors.unsqueeze(0).repeat(bz, 1, 1, 1).contiguous())
            ...
            if self.r_representation == 'quat':
                ...
                slot_R = torch.sigmoid(slot_R) * math.pi * 0.5 # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                # use slot r
                # slot_R = torch.sigmoid(slot_R) * math.pi * (45.0 / 180.0) # slot_R, math.pi --> slot_R
                # slot_R = torch.sigmoid(slot_R) * math.pi * 0.5 - 0.25 * math.pi # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                # slot_R: bz x n_s x 1 x tot_na -> bz x n_s x tot_na x 1
                slot_R = slot_R.contiguous().view(bz, self.num_slots, 1, cur_kanchor, -1).contiguous()
                to_add_angles = torch.tensor([0., 0.5 * math.pi, 1.0 * math.pi, 1.5 * math.pi], dtype=torch.float32).cuda().unsqueeze(0).unsqueeze(0).unsqueeze(0).unsqueeze(0)
                slot_R = slot_R + to_add_angles
                slot_R = slot_R.contiguous().view(bz, self.num_slots, 1, -1).contiguous()
                # slot_R = torch.sigmoid(slot_R) * math.pi # * 0.5 # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                # use
                if self.shape_type == 'drawer':
                    #### Set slot_R to identity matrix for the drawer category ####
                    slot_R = torch.eye(3, dtype=torch.float32).cuda().unsqueeze(0).unsqueeze(0).unsqueeze(0).contiguous().repeat(bz, self.num_slots, cur_kanchor * planeconv_kanchor, 1, 1).contiguous()
                    #### Set slot_R to identity matrix for the drawer category ####
                    # identity matrix for drawer?
                    # slot_R = torch.sigmoid(slot_R) * math.pi * 2.0 # * 0.5 # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                    # slot_R = compute_rotation_matrix_from_angle(
                    #     cur_anchors,  #
                    #     safe_transpose(slot_R, -1, -2).view(bz * self.num_slots, cur_kanchor, -1),
                    ...
                    slot_R = compute_rotation_matrix_from_angle(
                        self.tot_anchors, #
                        safe_transpose(slot_R, -1, -2).view(bz * self.num_slots, cur_kanchor * planeconv_kanchor, -1), defined_axis=defined_axis
                    ).contiguous().view(bz, self.num_slots, cur_kanchor * planeconv_kanchor, 3, 3).contiguous()
            ...
            # Get real rotations for each slot in each space-plane anchor
            slot_R = torch.matmul(self.tot_anchors.unsqueeze(0).unsqueeze(0), slot_R)
            ...
            ### Use one selected mode ### # to complete this thing?
            # k = 1
            ...
            k = self.kpconv_kanchor * planeconv_kanchor if self.sel_mode_trans is None else 1
            ...
            orbit_slot_dist_ori_recon = orbit_slot_dist_ori_recon.contiguous().view(bz, self.num_slots, cur_kanchor, planeconv_kanchor).contiguous()
            # orbit_slot_dist_ori_recon: bz x n_s x na
            # orbit_slot_dist_ori_recon, selected_slot_plane_anchors = torch.min(orbit_slot_dist_ori_recon, dim=-1)
            orbit_slot_dist_ori_recon, selected_slot_plane_anchors = torch.min(orbit_slot_dist_ori_recon[..., :1], dim=-1)
            ...
            if self.slot_single_mode == 1:
                ...
                selected_slot_plane_anchors = batched_index_select(values=selected_slot_plane_anchors, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(-1).contiguous()
            else:
                # orbit_slot_dist_ori_recon = minn_dist_ori_to_recon_hard + minn_dist_recon_to_ori
                ...
                selected_slot_plane_anchors = batched_index_select(values=selected_slot_plane_anchors,
            ...
            orbit_slot_dist_ori_recon_all_pts = orbit_slot_dist_ori_recon_all_pts.contiguous().view(bz, self.num_slots, cur_kanchor, planeconv_kanchor).contiguous()
            # orbit_slot_dist_ori_recon_all_pts: bz x n_s x na x plane_na -> bz x n_s x plane_na
            ...
            # orbit_slot_dist_ori_recon_all_pts: bz x n_s x plane_na -> bz x n_s
            orbit_slot_dist_ori_recon_all_pts = batched_index_select(values=orbit_slot_dist_ori_recon_all_pts, indices=selected_slot_plane_anchors.unsqueeze(-1), dim=2).squeeze(-1)
            ...
            transformed_pts = transformed_pts.contiguous().view(bz, self.num_slots, cur_kanchor, planeconv_kanchor, self.recon_part_M, 3).contiguous()
            transformed_pts = batched_index_select(transformed_pts, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(2)
            transformed_pts = batched_index_select(transformed_pts, indices=selected_slot_plane_anchors.unsqueeze(-1), dim=2).squeeze(2)
            ...
            slot_orbits = slot_orbits * planeconv_kanchor + selected_slot_plane_anchors
            ...
            if self.sel_mode is not None and self.slot_single_mode:
                # selected_glb_anchor: bz x n_s x 3 x 3
                selected_glb_anchor = batched_index_select(self.tot_anchors, indices=slot_orbits.long(), dim=0)[:, 0]
    ...
    def forward(self, x, pose, ori_pc=None, rlabel=None, nn_inter=2, pose_segs=None, canon_pc=None, normals=None, canon_normals=None):
        ...
        ### If use identity pose for convolution --- regular setting ###
        cur_estimated_pose = init_pose
        ### If use ground-truth pose --- an oracle setting for pose-aware convolution ###
...
def build_model(opt,
    ...
    use_2d = True if opt.equi_settings.use_2d == 1 else False
    # nmasks = 2
    sampling_ratio = 0.8
    sampling_density = 0.5
    kernel_multiplier = 2
    ...
    for i, block in enumerate(mlps):
        ...
        for j, dim_out in enumerate(block):
            ...
            kpconv_conv_param = {
                'type': 'inter_block', # we only use inter convolution in the kpconv module? aleilei
                'args': {
                    'dim_in': dim_in,
                    'dim_out': dim_out,
                    'kernel_size': kernel_size,
