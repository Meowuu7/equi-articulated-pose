...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.use_inv_angles = params['general']['use_inv_angles']
        ...
        self.inv_pooling_method = inv_pooling_method # invariant pooling method
        ...
        for i_s in range(self.num_slots):
            ...
            self.slot_outblock.append(
                Mso3.InvOutBlockOursWithMask(params['outblock'], norm=1, pooling_method='attention', use_pointnet=True, use_abs_pos=False)
            )

        # Inv output block for two parts
        if self.use_inv_angles == 1:
        ...
        # global output block... # global inv
        ...
        for i_s in range(self.num_slots):
            if self.recon_prior == 4:
                ...
                # Use cuboic for points reguralization # cuboid
                self.slot_shp_recon_net.append(
        ...
        for i_s in range(self.num_slots):
            ...
            if not self.pred_t: # only rotation & no translation
                ...
                # if self.use_inv_angles == 0 or self.use_inv_angles == 1:
                if self.use_inv_angles == 0:
                    if self.use_inv_angles == 0:
                        num_in_channels = None
                    else:
                        num_in_channels = self.inv_out_dim
                    ...
                    self.slot_trans_outblk_RT.append(
                        cur_slot_trans_outblk(params['outblock'], norm=1, pooling_method='max',
                                              ...
                                              # global scalar?
                                              use_anchors=False,
                                              feat_mode_num=self.kanchor, num_heads=1, representation=self.r_representation, num_in_channels=num_in_channels)
                    )
                else:
                    ...
                    self.slot_trans_outblk_RT.append(
                        cur_slot_trans_outblk(params['outblock'], norm=1, pooling_method='max',
                                              ...
                                              representation=self.r_representation,
                                              c_in_rot=c_in_rot, c_in_trans=c_in_trans)
                    )
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        if self.stage == 0:
            ...
            if isinstance(self.glb_trans_outblock_RT, SO3OutBlockRTWithMask) and glb_T is not None:
                ...
                # add xyzs as offset
                # Use the mean of all points for translation...
            ...
            if ori_pc is not None:
                out_feats['ori_pc'] = safe_transpose(ori_pc, -1, -2).detach().cpu().numpy()
            ...
            for i_s in range(self.num_slots):
                ...
                for i_bz in range(bz):
                    ...
                    if i_s in tot_seg_to_idxes[i_bz]:
                        n_pts_cur_slot = tot_seg_to_idxes[i_bz][i_s].size(0)
                    ...
                    if self.use_inv_angles == 1:
                        ...
                        pair_cur_bz_cur_slot_inv_feats, pair_cur_bz_cur_slot_orbit_confidence = self.pair_slot_outblock[i_s](
                            pair_cur_bz_cur_slot_x,
                            mask=None)

                    if self.use_inv_angles == 1:
                        ''' If we use invariant features for slots' angle decoding '''
                        # Get expanded slot's inv feats
                        expanded_cur_slot_inv_feats = pair_cur_bz_cur_slot_inv_feats.contiguous().unsqueeze(-1).unsqueeze(
                            -1).repeat(1, 1, n_pts_cur_slot, x.feats.size(-1))
                        # expanded_cur_slot_inv_feats = cur_bz_cur_slot_inv_feats.contiguous().unsqueeze(
                        #     -1).unsqueeze(
                        #     -1).repeat(1, 1, x.xyz.size(-1), x.feats.size(-1))
                        cur_bz_cur_slot_x = sptk.SphericalPointCloud(safe_transpose(cur_bz_cur_slot_xyz, -1, -2),
                                                                     expanded_cur_slot_inv_feats, x.anchors)
                        ''' If we use invariant features for slots' angle decoding '''

                    # cur_bz_cur_slot_inv_feats: bz x dim
                    # cur_bz_cur_slot_inv_
            ...
            if self.r_representation == 'quat':
                ...
                # slot_R = torch.sigmoid(slot_R) * math.pi * 0.5 # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                slot_R = torch.sigmoid(slot_R) * math.pi * 0.5 # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                # slot_R = torch.sigmoid(slot_R) * math.pi * 0.3 # * (36.0 / 180.0) # slot_R, math.pi --> slot_R
                # slot_R = torch.sigmoid(slot_R) * math.pi * 0.5 - 0.25 * math.pi # * (36.0 / 180.0) # slot_R, math.pi --> slot_R

                # if self.use_inv_angles == 1:
                #     slot_R[:, 0] = slot_R[:, 0] * 0.0

                slot_R_angles = slot_R.detach().clone()
            ...
            # ori_to_recon_dist: bz x N # calculate chamfer distance...
            ...
            tot_recon_loss = glb_recon_ori_dist * self.glb_recon_factor + (slot_dist_ori_recon) * self.slot_recon_factor
            ...
            if slot_R_angles is not None:
                # slot_R_angles: bz x n_s x 1 x na
                # print(slot_R_angles.size())
                selected_slot_R_angles = batched_index_select(slot_R_angles, indices=slot_orbits.unsqueeze(-1).long(), dim=2).squeeze(-1)
                # selected_slor_R_angles: bz x n_s --> Chosen angle of each slot
                out_feats['slot_R_angles'] = selected_slot_R_angles.detach().cpu().numpy()
            else:
                out_feats['slot_R_angles'] = None
            print(f"slot orbits, {slot_orbits.detach().cpu().numpy()}")
            out_feats['slot_orbits'] = slot_orbits.detach().cpu().numpy()
...
def build_model(opt,
    ...
    params['general'] = {
        ...
        'use_inv_angles': opt.equi_settings.use_inv_angles,
