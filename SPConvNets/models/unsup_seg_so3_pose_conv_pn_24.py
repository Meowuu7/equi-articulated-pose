...
from model_util import farthest_point_sampling, DecoderFC, DecoderFCAtlas
...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        # glb_offset: bz x 3 x 1
        glb_offset = torch.mean(x, dim=-1, keepdim=True)
        ...
        for i_s in range(self.num_slots):
            ...
            # cur_slot_inv_feats, orbit selection confidence
            cur_slot_inv_feats, cur_slot_orbit_confidence = self.slot_outblock[i_s](x, mask=None, soft_mask=None)
            # need not to mask anything...
            cur_slot_output_RT = self.slot_trans_outblk_RT[i_s](x, mask=None, anchors=self.anchors.unsqueeze(0).repeat(bz, 1, 1).contiguous())
            cur_slot_R = cur_slot_output_RT['R']
            cur_slot_T = cur_slot_output_RT['T']

            # Add oribt selection loss
        ...
        if isinstance(self.slot_trans_outblk_RT[0], SO3OutBlockRTWithMask):
            ...
            # slot_T: bz x n_s x 3 x na
            slot_T = slot_T + avg_offset.unsqueeze(-2).unsqueeze(-1)
        ...
        # distance between ori_pts and transformed_pts
        # ori_pts: bz x 3 x N --> bz x N x 3 --> bz x n_s x na x M x N
        # transformed points to original points
        dist_ori_transformed = torch.sum((transformed_pts.unsqueeze(-2) - safe_transpose(ori_pts, -1, -2).unsqueeze(1).unsqueeze(1).unsqueeze(1)) ** 2, dim=-1)
        # dist_trans_to_ori: bz x n_s x na x M
        dist_trans_to_ori, _ = torch.min(dist_ori_transformed, dim=-1)
        # avg_..: bz x n_s x na
        avg_dist_trans_to_ori = dist_trans_to_ori.mean(dim=-1)
        # slot_orbits: bz x n_s
        minn_dist_trans_to_ori, slot_orbits = torch.min(avg_dist_trans_to_ori, dim=-1)

        # selected_trans_pts: bz x n_s x M x 3
        selected_trans_pts = batched_index_select(values=transformed_pts, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(2)
        # dist_ori_transformed: bz x n_s x M x N
        dist_ori_transformed = batched_index_select(values=dist_ori_transformed, indices=slot_orbits.unsqueeze(-1), dim=2).squeeze(2)
        # dist_ori_to_trans: bz x n_s x N
        dist_ori_to_trans, _ = torch.min(dist_ori_transformed, dim=-2)
        # dist_ori_to_trans: bz x N; pts_labels: bz x N
        dist_ori_to_trans, pts_labels = torch.min(dist_ori_to_trans, dim=-2)

        avg_dist_ori_to_trans = dist_ori_to_trans.mean(dim=-1)
        # hard_one_hot_labels: bz x N x n_s
        hard_one_hot_labels = torch.eye(self.num_slots, dtype=torch.float32).cuda()[pts_labels]
        # hard_slot_indicators: bz x n_s
        hard_slot_indicators = (torch.sum(hard_one_hot_labels, dim=-2) > 0.5).float()

        # expanded_hard_one_hot_labels: bz x n_s x M x N
        expanded_hard_one_hot_labels = safe_transpose(hard_one_hot_labels, -1, -2).unsqueeze(-2).contiguous().repeat(1, 1, self.recon_part_M, 1)
        dist_ori_transformed[expanded_hard_one_hot_labels < 0.5] = 999999.0
        dist_trans_to_ori, _ = torch.min(dist_ori_transformed, dim=-1)
        avg_dist_trans_to_ori = dist_trans_to_ori.mean(dim=-1)
        # avg_dist_trans_to_ori: bz
        avg_dist_trans_to_ori = torch.sum(avg_dist_trans_to_ori * hard_slot_indicators, dim=-1) / torch.sum(hard_slot_indicators, dim=-1)
        slot_dist_ori_recon = avg_dist_trans_to_ori + avg_dist_ori_to_trans

        ''' Get filtered transformed points '''
        filtered_transformed_pts = selected_trans_pts * hard_slot_indicators.unsqueeze(-1).unsqueeze(-1)
        ...
        if cur_iter == 0:
            ...
            self.attn_saved_1 =  safe_transpose(hard_one_hot_labels, -1, -2)
        ...
        if gt_pose is not None:
            ...
            gt_transformed_ori_pts = torch.matmul(gt_inv_R,
                                                  (safe_transpose(ori_pts, -1, -2) - (gt_T - glb_offset.squeeze(-1).unsqueeze(-2))).unsqueeze(-1)).squeeze(-1)
