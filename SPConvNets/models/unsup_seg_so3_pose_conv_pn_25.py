...
class ClsSO3ConvModel(nn.Module):  # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        self.kpconv_kanchor = params['outblock']['kpconv_kanchor']
        ...
        if self.kpconv_kanchor == 1:
            ...
            # Get kpconv-anchors via the value of kpconv_kanchor
        ...
        if self.gt_oracle_seg == 0:
            ...
            self.slot_attention = SlotAttention(num_slots=params['outblock']['k'],
                                                dim=self.attn_in_dim, hidden_dim=self.inv_out_dim, iters=self.slot_iters)
        ...
        self.glb_outblock = Mso3.InvOutBlockOursWithMask(params['outblock'], norm=1, pooling_method='attention', use_pointnet=True)
        ...
        if not self.glb_pred_t:
            self.glb_trans_outblock_RT = SO3OutBlockRWithMask(params['outblock'], norm=1, pooling_method='max', pred_t=self.glb_pred_t, feat_mode_num=self.kanchor)
        else:
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
        ...
        #### Get cur_kanchor via the number of iteration ####
        ...
        ''' Calculate global rotations and translations '''
        if cur_iter == 0:
            ...
            ''' Get glb_R and glb_T '''
            # compute global rotation from quaternion
            ...
            transformed_glb_recon_pts = safe_transpose(torch.matmul(glb_R, glb_recon_canon_pts.unsqueeze(1)), -1, -2) + glb_T.unsqueeze(-2)
            ...
            chamfer_recon_to_ori, chamfer_ori_to_recon = safe_chamfer_dist_call(transformed_glb_recon_pts.contiguous().view(bz * cur_kanchor, transformed_glb_recon_pts.size(-2), 3), expanded_ori_pts.contiguous().view(bz * cur_kanchor, npoints, 3), self.chamfer_dist)
        ...
        if self.recon_prior == 5 and cur_iter > 0:
            ...
            #         slot_R, slot_T, ori_pts, normals, slot_cuboic_recon_pts, slot_cuboic_R, hard_one_hot_labels,
            #         attn_ori
            #     )
            ''' Get cuboid reconstruction: whether to consider normals in the calculation process or not '''

            ''' Get cuboid reconstruction loss: chamfer distance based loss '''
            slot_recon_cuboic_constraint_loss, cuboid_sampled_pts = get_cuboic_constraint_loss_cd_based(
                slot_R, slot_T, ori_pts, slot_cuboic_recon_pts, slot_cuboic_R, hard_one_hot_labels, attn_ori
            )
            ''' Get cuboid reconstruction loss: chamfer distance based loss '''
        ...
        if self.recon_prior == 5:
            ...
            if cur_iter > 0 and cuboid_sampled_pts is not None:
                #### Register cuboid sampled points in the output feature dictionary ####
                out_feats['cuboid_sampled_pts'] = cuboid_sampled_pts.detach().cpu().numpy()
        ...
        tmp_T = torch.matmul(safe_transpose(selected_glb_R.detach(), -1, -2).unsqueeze(1), (gt_pose[:, :, :3, 3] - selected_glb_T.detach().unsqueeze(1)).unsqueeze(-1)).squeeze(-1)
        # Get GT Pose for each point
        gt_pose = torch.cat([tmp_R, tmp_T.unsqueeze(-1)], dim=-1)
        # Get GT Pose for
        gt_pose = torch.cat([gt_pose, torch.zeros((tmp_R.size(0), tmp_R.size(1), 1, 4), dtype=torch.float32).cuda()], dim=-2)
    ...
    def forward(self, x, pose, ori_pc=None, rlabel=None, nn_inter=2, pose_segs=None, canon_pc=None, normals=None, canon_normals=None):
        ...
        for i in range(self.num_iters):
            cur_reconstructed_loss_orbit, attn, cur_estimated_pose, cur_out_feats, cur_selected_pts_orbit, cur_gt_pose = self.forward_one_iter(
                cur_transformed_points, cur_estimated_pose, ori_pc=ori_pc, rlabel=rlabel, cur_iter=i, gt_pose=cur_gt_pose,
...
def build_model(opt,
    ...
    kpconv_na = opt.equi_settings.kpconv_kanchor
    nmasks = opt.nmasks
