...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    ...
    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None): # rotation label
        ...
        # todo: add glb pose supervision
        # from x_out to decoded whole shape
        # from var out features to weights?
        # perform orbit selection?
        # use the selected orbit to align with other orbits?

        # bz
        glb_selected_orbit = torch.argmax(orbit_attn_weights, dim=-1)
        # # glb_selected_anchor: 3 x 3
        # glb_selected_anchor = self.anchors[glb_selected_orbit]
        glb_selected_anchor = batched_index_select(values=self.anchors, indices=glb_selected_orbit.unsqueeze(-1), dim=0)
        # glb_selected_anchor: bz x 3 x 3
        glb_selected_anchor = glb_selected_anchor.squeeze(1)

        # that lies in the plane for further selection
        # we should determine an axis?
        # rotation is then put one one

        # glb_rot: bz x 3 x 3
        glb_rot = gt_pose_segs[:, 0, :3, :3]
        # gt_axis: bz x 3
        gt_axis = torch.matmul(glb_rot, torch.tensor([0, 1., 0], dtype=torch.float32).cuda().unsqueeze(0).unsqueeze(-1)).squeeze(-1)

        # x_out
        # predicted_axis: bz x 3
        predicted_axis = self.axis_prediction(x_out.unsqueeze(-1).unsqueeze(-1))
        predicted_axis = predicted_axis / torch.clamp(torch.norm(predicted_axis, p=2, keepdim=True, dim=-1), min=1e-8)
        # todo: add axis loss
        predicted_axis = predicted_axis.squeeze(-1).squeeze(-1)

        # predicted_axis = torch.matmul(glb_selected_anchor, predicted_axis.unsqueeze(-1)).squeeze(-1)
        predicted_axis = torch.matmul(glb_rot, predicted_axis.unsqueeze(-1)).squeeze(-1)

        axis_pred_axis_loss = torch.sum((predicted_axis - gt_axis) ** 2, dim=-1).mean()

        predicted_axis = gt_axis

        aa_x, aa_y, aa_z = self.anchors[..., 2, 1] - self.anchors[..., 1, 2], self.anchors[..., 0, 2] - self.anchors[..., 2, 0], self.anchors[..., 1, 0] - self.anchors[..., 0, 1]
        # anchor_axises: k x 3
        anchor_axises = torch.cat([aa_x.unsqueeze(-1), aa_y.unsqueeze(-1), aa_z.unsqueeze(-1)], dim=-1)
        anchor_axises = anchor_axises / torch.clamp(torch.norm(anchor_axises, p=2, dim=-1, keepdim=True), min=1e-8)
        # dot_predicted_anchors_axises: bz x k
        dot_predicted_anchors_axises = torch.sum(predicted_axis.unsqueeze(1) * anchor_axises.unsqueeze(0), dim=-1)
        # selected_anchor_idxes: bz x 5
        selected_anchor_axises, selected_anchor_idxes = torch.topk(dot_predicted_anchors_axises, dim=-1, k=5, largest=True)

        # how to predict global transformation?
        ...
        # selected_anchor_idxes: bz x 5
        slot_expanded_anchor_idxes = selected_anchor_idxes.unsqueeze(1).repeat(1, self.num_slots, 1).contiguous()
