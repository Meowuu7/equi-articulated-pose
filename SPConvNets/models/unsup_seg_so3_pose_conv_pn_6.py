...
from model_util import farthest_point_sampling, DecoderFC, initialize_model_modules
...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    def __init__(self, params):
        ...
        ''' Construct global rotation factorization modules '''
        ...
        ''' Construct model for global rotation factorization '''
        # output features: bz x dim
        self.glb_outblock = Mso3.ClsOutBlockPointnet(params['outblock'], )

        ''' Construct global shape docoder '''
        self.glb_shp_reconstruction_net = nn.Sequential(
            ...
            nn.BatchNorm2d(num_features=1024),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(in_channels=1024, out_channels=self.npoints * 3,
                      kernel_size=(1, 1), stride=(1, 1), bias=True),
            nn.Sigmoid()
        )
        initialize_model_modules(self.glb_shp_reconstruction_net)
        ...
        ''' Construct invariant feature extraction network for per-point invariant feature extraction '''
        if self.inv_attn == 1:
        ...
        initialize_model_modules(self.part_reconstruction_net)
        ...
        for i_s in range(self.num_slots):
            ...
            if self.use_sigmoid == 1:
                tmp_modules.append(nn.Sigmoid())
            self.category_part_reconstruction_net.append(nn.Sequential(*tmp_modules))

        ''' Construct category-common part prior parameters '''
        self.category_part_prior_params = nn.ParameterList()
        for i_s in range(self.num_slots):
            ## Default `requires_grad` is True for Parameters
        ...
        for i_s in range(self.num_slots):
            ...
            self.deformation_flow_predict_per_part_net.append(
                nn.Sequential(
                    nn.Conv2d(in_channels=self.encoded_feat_dim + 3, out_channels=self.encoded_feat_dim // 2, kernel_size=(1, 1),
                              stride=(1, 1), bias=True),
                    nn.BatchNorm2d(num_features=self.encoded_feat_dim // 2),
                    # nn.ReLU(),
                    nn.LeakyReLU(inplace=True),
        ...
        ''' Initialize deformation flow prediction networks for slots '''
        initialize_model_modules(self.deformation_flow_predict_per_part_net)
        ...
        if self.recon_prior in [3, 4]:
            ...
            initialize_model_modules(self.category_part_reconstruction_net_v2)

        ''' Slot prior parameters '''
    ...
    def apply_part_deformation_flow_net_instance(self, category_pts, feats):
        ...
        for i_s, mod in enumerate(self.deformation_flow_predict_per_part_net):
            ...
            if self.use_flow_reg == 0:
                ...
                cur_slot_predicted_flow = cur_slot_predicted_flow * 0.20 # 0.2 for oven
    ...
    def forward_global(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None,
                         canon_pc=None, selected_pts_orbit=None):
        # x: bz x 3 x npoints
        ori_pts = x.clone()
        ...
        for block_i, block in enumerate(self.glb_backbone):
            x = block(x, seg=seg)
        torch.cuda.empty_cache()
        # inv_feats: bz x dim; var_feats: bz x dim x na
        inv_feats, _, var_feats = self.glb_outblock(x)
        # predicted_pts: bz x npoints x 3
        # print( inv_feats.size())
        predicted_pts = self.glb_shp_reconstruction_net(inv_feats.unsqueeze(-1).unsqueeze(-1))
        predicted_pts = predicted_pts.squeeze(-1).squeeze(-1).contiguous().view(bz, npoints, 3)
        # predicted_trans: bz x 7 x na
        predicted_trans = self.glb_transformation_prediction(var_feats.unsqueeze(-1))
        predicted_trans = predicted_trans.squeeze(-1)
        pred_R, pred_T = predicted_trans[:, :4, :], predicted_trans[:, 4:, :]

        # pred_R: bz x na x 3 x 3
        pred_R = compute_rotation_matrix_from_quaternion(
            pred_R.contiguous().permute(0, 2, 1).contiguous().view(-1, 4)).contiguous().view(bz, self.kanchor, 3, 3)

        # pred_R: bz x na x 3 x 3
        pred_R = torch.matmul(self.anchors.unsqueeze(0), pred_R)

        ''' From predicted T to the real translation vector --- part rotation modeling '''
        # pred_res_T: bz x na x 3
        # we do not use global pred_T here
        pred_T = pred_T.contiguous().permute(0, 2, 1).contiguous()
        # rotated_pts: bz x na x 3 x npoints
        rotated_pts = torch.matmul(pred_R, safe_transpose(predicted_pts, 1, 2).unsqueeze(1))
        # rotated_pts: bz x na x npoints x 3
        rotated_pts = safe_transpose(rotated_pts, 2, 3)
        # glb_chamfer_dist: bz x na x npoints x npoints (ori, transformed)
        glb_chamfer_dist = torch.sum((safe_transpose(ori_pts, 1, 2).unsqueeze(1).unsqueeze(-2) - rotated_pts.unsqueeze(-3)) ** 2, dim=-1)
        # ori_to_trans: bz x na x npoints; trans_to_ori: bz x na x npoints
        glb_chamfer_dist_ori_to_trans, glb_chamfer_dist_trans_to_ori = torch.min(glb_chamfer_dist, dim=-1)[0], torch.min(glb_chamfer_dist, dim=-2)[0]
        # bz x na
        glb_chamfer_dist = glb_chamfer_dist_ori_to_trans.sum(dim=-1) + glb_chamfer_dist_trans_to_ori.sum(dim=-1)
        # dist: bz; minn_orbit: bz
        minn_glb_chamfer_dist, minn_orbit = torch.min(glb_chamfer_dist, dim=-1)
        # selected_pred_R: bz x 3 x 3
        selected_pred_R = batched_index_select(values=pred_R, indices=minn_orbit.unsqueeze(-1), dim=1).squeeze(1)

        glb_rot_transformed_pts = torch.matmul(safe_transpose(selected_pred_R, 1, 2), ori_pts)

        # glb_rot_transformed_pts: transformed by rotation;
        # minn_glb_chamfer_dist: selected chamfer distance;
        # selected_pred_R: selected rotation
        return glb_rot_transformed_pts, minn_glb_chamfer_dist, selected_pred_R

    def forward_one_iter(self, x, pose, ori_pc=None, rlabel=None, cur_iter=0, gt_pose=None, gt_pose_segs=None, canon_pc=None, selected_pts_orbit=None, glb_rot=None): # rotation label

        assert glb_rot is not None
        ...
        ''' Chamfer distance call version '''
        # expaned_transformed_slot_pts = transformed_slot_pts.contiguous().view(bz * self.num_slots * self.kanchor,
        #                                                                       self.recon_part_M, 3).contiguous()
        # expaned_ori_pts = ori_pts.unsqueeze(1).unsqueeze(1).repeat(1, self.num_slots, self.kanchor, 1, 1).contiguous()
        # # hard_one_hot_labels: bz x n_s x N --> bz x n_s x na x N x 3
        # expaned_hard_one_hot_labels = hard_one_hot_labels.unsqueeze(-1).unsqueeze(2).repeat(1, 1, self.kanchor, 1,
        #                                                                                     3).contiguous()
        # expaned_ori_pts[expaned_hard_one_hot_labels < 0.5] = 99999.9
        # expaned_ori_pts = expaned_ori_pts.view(bz * self.num_slots * self.kanchor, npoints, 3).contiguous()
        # # chamfer_slot_to_ori_dists: (bz x n_s x na) x M; chamfer_ori_to_slot_dists: (bz x n_s x na) x N
        # chamfer_slot_to_ori_dists, chamfer_ori_to_slot_dists = safe_chamfer_dist_call(expaned_transformed_slot_pts,
        #                                                                               expaned_ori_pts,
        #                                                                               chamfer_func=self.chamfer_dist)
        # chamfer_slot_to_ori_dists = chamfer_slot_to_ori_dists.contiguous().view(bz, self.num_slots, self.kanchor,
        #                                                                         self.recon_part_M).contiguous().permute(
        #     0, 1, 3, 2).contiguous()
        # chamfer_ori_to_slot_dists = chamfer_ori_to_slot_dists.contiguous().view(bz, self.num_slots, self.kanchor,
        #                                                                         npoints).contiguous().permute(0, 1, 3,
        #                                                                                                       2).contiguous()
        #
        # dist_chamfer_recon_slot = chamfer_slot_to_ori_dists
        # dist_chamfer_recon_ori = chamfer_ori_to_slot_dists
        #
        # dist_chamfer_recon_slot[dist_chamfer_recon_slot > 100.] = 0.
        # dist_chamfer_recon_ori[dist_chamfer_recon_ori > 100.] = 0.
        #
        # dist_chamfer_recon_slot = torch.sum(dist_chamfer_recon_slot, dim=2)
        ...
        if self.soft_attn == 0:
            ...
            # sharp_soft_labels_slot = hard_one_hot_labels * soft_labels_slot
            # dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * sharp_soft_labels_slot.unsqueeze(-1), dim=2) / torch.clamp(torch.sum(sharp_soft_labels_slot.unsqueeze(-1), dim=2), min=1e-9)
            dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * hard_one_hot_labels.unsqueeze(-1), dim=2) / torch.clamp(torch.sum(hard_one_hot_labels.unsqueeze(-1), dim=2), min=1e-9)
            dist_chamfer_recon_ori_to_slot = torch.sum(dist_chamfer_recon_ori * hard_one_hot_labels.unsqueeze(-1), dim=2) # / torch.clamp(torch.sum(hard_one_hot_labels.unsqueeze(-1), dim=2), min=1e-9)
        ...
        # sum_slot_soft_nn_pts = torch.sum(attn_ori, dim=-1)
        # sum_slot_soft_nn_pts = torch.sum(hard_one_hot_labels, dim=-1)
        # inv_sum_slot_soft_nn_pts = 1. / torch.clamp(torch.sqrt(sum_slot_soft_nn_pts), min=1e-9)
        # # inv_sum_slot_soft_nn_pts[sum_slot_soft_nn_pts]
        # # inv_sum_slot_soft_nn_pts = torch.softmax(inv_sum_slot_soft_nn_pts, dim=-1)
        # inv_sum_slot_soft_nn_pts[sum_slot_soft_nn_pts < 1.] = 0.
        # inv_sum_slot_soft_nn_pts = inv_sum_slot_soft_nn_pts / torch.sum(inv_sum_slot_soft_nn_pts, dim=-1, keepdim=True)
        #
        # # inv_sum_slot_soft_nn_pts = torch.softmax(inv_sum_slot_soft_nn_pts, dim=-1)
        # #### ####
        # # print(inv_sum_slot_soft_nn_pts[0].tolist(), sum_slot_soft_nn_pts[0].tolist())
        # avg_dist_chamfer_recon_slot_ori = torch.sum(inv_sum_slot_soft_nn_pts * dist_chamfer_recon_slot_ori, dim=-1)
        ...
        # glb_recon_factor = 3.0
        slot_recon_factor = 4.0 # 3.0 # .0
        ...
        selected_ori_pred_T = torch.matmul(glb_rot.unsqueeze(1), selected_ori_pred_T.unsqueeze(-1)).unsqueeze(-1)
        selected_pred_T_saved = selected_pred_T.detach().clone()
        selected_pred_T_saved = torch.matmul(glb_rot.unsqueeze(1), selected_pred_T_saved.unsqueeze(-1)).unsqueeze(-1)
        # selected_pred_R: bz x N x 3 x 3
        ...
        rel_selected_pred_T = selected_pred_T.clone()
        # print(glb_rot.size(), selected_pred_T.size())
        selected_pred_T = torch.matmul(glb_rot.unsqueeze(1), selected_pred_T.unsqueeze(-1)).squeeze(-1)
        ...
        # bz x 3 x 3 xxxx bz x 3 x npoints --> bz x 3 x npoints
        ori_pts_transformed_glb = torch.matmul(glb_rot, safe_transpose(ori_pts, 1, 2))
        ori_pts_transformed_glb = safe_transpose(ori_pts_transformed_glb, 1, 2)
        transformed_ori_pts = torch.matmul(selected_inv_pred_R, (ori_pts_transformed_glb - selected_pred_T).unsqueeze(-1)).squeeze(-1)
        ...
        if gt_pose is not None:
            ...
            gt_transformed_ori_pts = torch.matmul(gt_inv_R, (ori_pts_transformed_glb - gt_T).unsqueeze(-1)).squeeze(-1)
        ...
        pred_pose = torch.cat([rel_selected_pred_R, torch.zeros((bz, npoints, 3, 1), dtype=torch.float32).cuda()],
                              dim=-1)
        ...
        out_feats['pred_R_slots'] = selected_pred_R_saved.detach().cpu().numpy()
    ...
    def forward(self, x, pose, ori_pc=None, rlabel=None, nn_inter=2, pose_segs=None, canon_pc=None):
        ...
        glb_rot_transformed_pts, minn_glb_chamfer_dist, selected_pred_R = self.forward_global(cur_transformed_points, cur_estimated_pose, ori_pc=ori_pc, rlabel=rlabel, cur_iter=0, gt_pose=pose, gt_pose_segs=pose_segs, canon_pc=canon_pc, selected_pts_orbit=None)

        # out_feats = {}

        torch.cuda.empty_cache()

        # tot_loss += minn_glb_chamfer_dist.mean()

        out_feats_all_iters = {}
        cur_selected_pts_orbit = None
        for i in range(nn_inter):
            cur_reconstructed_loss_orbit, attn, cur_estimated_pose, cur_out_feats, cur_selected_pts_orbit = self.forward_one_iter(glb_rot_transformed_pts, cur_estimated_pose, ori_pc=ori_pc, rlabel=rlabel, cur_iter=i, gt_pose=pose, gt_pose_segs=pose_segs, canon_pc=canon_pc, selected_pts_orbit=cur_selected_pts_orbit, glb_rot=selected_pred_R)
            torch.cuda.empty_cache()
        ...
        return (tot_loss / nn_inter) + minn_glb_chamfer_dist.mean()
        # return minn_glb_chamfer_dist.mean() # for global alignment module testing
