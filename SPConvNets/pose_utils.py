...
def pose_fit(source, target, cfg, hard=True):  # [B, N, 3]
    ...
    with torch.no_grad():
        ...
        err = (target.reshape(B, 1, -1, 3, 1)  # [B, 1, N, 3, 1] --> [B, H, N, 3, 1]
               ...
               * torch.matmul(rotation.unsqueeze(-3),
                              source.reshape(B, 1, -1, 3, 1))  # [B, H, 1, 3, 3], [B, 1, N, 3, 1]
...
def compute_pose_diff(nocs_gt, nocs_pred, target, category):
    ...
    pose_info = {'r_gt': gt_rot, 't_gt': gt_trans, 's_gt': gt_model['scale'], 'r_pred': pred_rot, 't_pred': pred_trans,
                 's_pred': gt_model['scale']}
