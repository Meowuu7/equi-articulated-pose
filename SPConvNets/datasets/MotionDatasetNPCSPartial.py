...
class MotionDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.no_articulation == 0:
            ...
            for i_seg in range(len(cur_motion_attributes)):
                ...
                if cur_seg_motion_info["motion_type"] == "rotation" and (rot_1 == False or self.shape_type == "eyeglasses"):
                    ...
                    # part_ref_rots.append(np.reshape(canon_transformation_mtx[:3, :3], (1, 3, 3)))
                    part_ref_rots.append(np.reshape(transformation_mtx[0, :3, :3], (1, 3, 3))) # ref rots
            ...
            # here, important variables are transformed_vertices and canon_transformed_vertices
                ...
                tight_w = max(cur_seg_tri_vertices[:, 0]) - min(cur_seg_tri_vertices[:, 0])
                tight_l = max(cur_seg_tri_vertices[:, 1]) - min(cur_seg_tri_vertices[:, 1])
                tight_h = max(cur_seg_tri_vertices[:, 2]) - min(cur_seg_tri_vertices[:, 2])
                norm_factor = np.sqrt(1) / np.sqrt(tight_w ** 2 + tight_l ** 2 + tight_h ** 2)  # norm factor
                corner_pt_left = np.amin(cur_seg_tri_vertices[:, :3], axis=0, keepdims=True)
                corner_pt_right = np.amax(cur_seg_tri_vertices[:, :3], axis=0, keepdims=True)
        ...
        # npcs = pc + npcs_trans --- npcs - npcs_trans = pc; R * pc + t = npx
        ...
        for seg_label in seg_label_to_pts: #
            ...
            inv_trans_cur_seg_pts = np.matmul(np.transpose(cur_seg_rot, (1, 0)), np.transpose(cur_seg_trans_pts - np.reshape(cur_seg_trans, (1, 3)), (1, 0)))
            ...
            cur_npcs = (inv_trans_cur_seg_pts[:, :3] - corner_pts[0]) * norm_factor - 0.5 * (corner_pts[1] - corner_pts[0]) * norm_factor
        ...
        # if self.est_normals == 1:
        #     rt_dict['cur_normals'] = cur_normals.contiguous().transpose(0, 1).contiguous()
        #     rt_dict['cur_canon_normals'] = cur_canon_normals.contiguous().transpose(0, 1).contiguous()
