...
class MotionDataset(data.Dataset):
    def __init__(
        ...
        if self.shape_type == "oven":
            ...
            not_ok_shape_idxes = [50, 58, 34, 2, 18, 27, 11, 29, 45, 28, 10, 21, 7, 38, 9, 31, 62, 30, 37, 8, 1, 6, 39,
                                  52, 46, 41, 24, 12, 40, 13, 61, 59, 32, 35, 59]
            not_ok_shape_idxes = ["%.4d" % iii for iii in not_ok_shape_idxes]
            not_ok_shape_idx_to_va = {str(iii): 1 for iii in not_ok_shape_idxes}
            shape_idxes = [si for si in shape_idxes if si not in not_ok_shape_idx_to_va]
        elif self.shape_type == "eyeglasses":
            not_ok_shape_idxes = [42]
            not_ok_shape_idxes = [3, 33, 5, 2, 20, 11, 29, 16, 42, 28, 10, 19, 7, 38, 9, 30, 8, 1, 6, 41, 23, 15, 12,
                                  13, 14, 4, 32, 28, 17]
            not_ok_shape_idxes = ["%.4d" % iii for iii in not_ok_shape_idxes]
            not_ok_shape_idx_to_va = {str(iii): 1 for iii in not_ok_shape_idxes}
            shape_idxes = [si for si in shape_idxes if si not in not_ok_shape_idx_to_va]
    ...
    def __getitem__(self, index):
        ...
        if self.no_articulation == 0:
            ...
            for i_seg in range(len(cur_motion_attributes)):
                ...
                tight_w = max(cur_seg_pts[:, 0]) - min(cur_seg_pts[:, 0])
                tight_l = max(cur_seg_pts[:, 1]) - min(cur_seg_pts[:, 1])
                tight_h = max(cur_seg_pts[:, 2]) - min(cur_seg_pts[:, 2])
                norm_factor = np.sqrt(1) / np.sqrt(tight_w ** 2 + tight_l ** 2 + tight_h ** 2)
                corner_pt_left = np.amin(cur_seg_pts, axis=0, keepdims=True)
                corner_pt_right = np.amax(cur_seg_pts, axis=0, keepdims=True)
                cur_corner_pts = [corner_pt_left, corner_pt_right]

                # Get NPCS for current part
                # cur_npcs = (cur_seg_pts[:, :3] - cur_corner_pts[0]) * norm_factor + np.array([0.5, 0.5, 0.5]).reshape(1, 3) - 0.5 * (
                ...
                # cur_npcs_trans = -cur_corner_pts[0] * norm_factor + np.array([0.5, 0.5, 0.5]).reshape(1, 3) - 0.5 * (
                                        # cur_corner_pts[1] - cur_corner_pts[0]) * norm_factor
                # cur_npcs_trans = -cur_corner_pts[0] + np.array([0.5, 0.5, 0.5]).reshape(1, 3) / norm_factor - 0.5 * (
