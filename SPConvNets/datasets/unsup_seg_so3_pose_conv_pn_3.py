...
class ClsSO3ConvModel(nn.Module): # SO(3) equi-conv-network #
    ...
    def forward_one_iter(self, x, pose, rlabel=None, cur_iter=0, gt_pose=None): # rotation label
        ...
        hard_one_hot_labels_slot = hard_one_hot_labels / torch.clamp(hard_one_hot_labels.sum(dim=-2, keepdim=True),
                                                                     min=1e-9)
    ...
    def forward(self, x, pose, rlabel=None, nn_inter=2):
        ...
        for i in range(nn_inter):
            cur_reconstructed_loss_orbit, attn, cur_estimated_pose = self.forward_one_iter(cur_transformed_points, cur_estimated_pose, rlabel=rlabel, cur_iter=i, gt_pose=pose)
